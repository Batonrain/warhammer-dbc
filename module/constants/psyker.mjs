...
export const PSY_PATHS = {
  ...
  // Руны Сигиллитов (wdbc-fsl9, DoomBC — Психокеры-Жабы, стр. 101-102).
  //
  // `flag` — единственный Путь в списке, который ПОКАЗЫВАЕТСЯ не всем: без
  // Черты «Магия Сигиллитов» его нет в выпадающем списке вовсе (sheets/tabs/
  // psychic.mjs). Остальные Пути поля не несут и ведут себя как прежде —
  // псайкер без этой Черты не видит в окне манифестации ни одного изменения.
  //
  // `focusTest` — книжное «считается Best.Q Психофокусом на все дисциплины»:
  // это ровно тот же свободный тест W+0, что у Пути «Психофокус», качество
  // предмета в системе на него и так не влияет.
  //
  // `runeCost` — метка «этот Путь платит Рунами» (rules/sigillite-runes.mjs).
  //
  // `subPaths` (wdbc-qd6w) — единственный Путь в реестре, где книга прямо разрешает
  // СКЛАДЫВАТЬ механики нескольких других Путей одновременно, а не выбирать
  // один взаимоисключающе (обычное поведение всех прочих записей этого
  // реестра): «может использовать механику ВСЕХ следующих путей ОДНОВРЕМЕННО
  // (необязательно все сразу, псайкер может определять желаемые)» (стр.
  // 101-102). Ключи — записи ЭТОГО ЖЕ реестра (incantation/meditation/
  // unholy): их ePR/testMod/phenMod читаются напрямую через subPathTotals
  // ниже, вместо второй копии тех же чисел на записи sigillite.
  sigillite:     { label: "Руны Сигиллитов",       req: "Черта Sigillite Magic / Магия Сигиллитов", testMod: 0, ePR: 0, phenMod: 0,
                   flag: "psychicPath.sigillites.runeMagic",
                   runeMode: true, phenOnly99: true, focusTest: true, runeCost: true,
                   subPaths: ["incantation", "meditation", "unholy"],
                   note: "Только Безопасный/Обычный режим; Феномен лишь на 99. Цена — бPR психосилы × 2 Руны, считается и списывается сама. Отметьте любую комбинацию флажков ниже (Инкантация/Медитация/Нечестивые Символы) — они складываются кумулятивно, а не выбираются взаимоисключающе. Инкантация также поднимает действие манифестации на ступень (полудействие→полное) — это НЕ автоматизировано, отслеживайте вручную. НЕ автоматизировано и остаётся на столе: −30 всем попыткам обнаружить манифестацию, доп. −30 при варп-прорыве, доступ к Тауматургии." }
};

/**
 * Суммарный вклад отмеченных суб-механик Пути `pathKey` (сейчас — только
 * «Руны Сигиллитов», см. `subPaths` выше). Каждый ключ в `selectedKeys` —
 * запись ЭТОГО ЖЕ реестра PSY_PATHS; неизвестный ключ или ключ, не входящий
 * в `subPaths` этого Пути, тихо игнорируется (защита от рассинхрона разметки
 * окна с данными, а не жалоба в консоль — это не `when`-предикат книги, а
 * список чекбоксов одного конкретного диалога).
 *
 * Числа НЕ копируются — читаются напрямую из тех же записей PSY_PATHS,
 * которыми пользуется одиночный выбор Пути, поэтому правка книжных чисел
 * Инкантации/Медитации/Нечестивых Символов в одном месте меняет и обычный
 * выбор, и комбинацию Сигиллита разом.
 *
 * @param {string} pathKey
 * @param {string[]} [selectedKeys]
 * @returns {{ePR:number, testMod:number, phenMod:number, labels:string[]}}
 */
export function subPathTotals(pathKey, selectedKeys) {
  const allowed = new Set(PSY_PATHS[pathKey]?.subPaths || []);
  let ePR = 0, testMod = 0, phenMod = 0;
  const labels = [];
  for (const key of (Array.isArray(selectedKeys) ? selectedKeys : [])) {
    if (!allowed.has(key)) continue;
    const sub = PSY_PATHS[key];
    if (!sub) continue;
    ePR     += sub.ePR || 0;
    testMod += sub.testMod || 0;
    phenMod += sub.phenMod || 0;
    labels.push(sub.label);
  }
  return { ePR, testMod, phenMod, labels };
}
