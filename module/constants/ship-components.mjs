...
// Снаряжение корабля — предметы-грузы (тип "cargo"). o: { ct, lc, r, desc }
function E(folder, name, o) {
  return {
    name, type: "cargo", img: IMG, folder,
    system: {
      cargoType: o.ct || "equipment", lc: o.lc ?? 1, quantity: o.qty ?? 1, quality: "common",
      rarity: o.r ?? 0, baseRarity: "", description: o.desc || ""
    }
  };
}

const SNAR_SHIP = ["Снаряжение", "Корабельное"];
const SNAR_WAR  = ["Снаряжение", "Боеголовки"];
const SNAR_NAV  = ["Снаряжение", "Системы наведения"];
const SNAR_MYST = ["Снаряжение", "Мистическое"];
const SNAR_STAT = ["Снаряжение", "Стационарное"];
...
// ════════════════════════════════════════════════════════════════════════
//  Снаряжение корабля (грузы) — папка «Снаряжение» в компендиуме.
// ════════════════════════════════════════════════════════════════════════
export const SHIP_EQUIPMENT = [
  // Корабельное
  E(SNAR_SHIP, "Снаряд нова-орудия", { ct:"ammo", lc:1, r:3, desc:"Боезапас нова-орудия (обычно 20 на узел). Реквизиция вне мира-кузни: −10 (плюс прочие модификаторы)." }),
  E(SNAR_SHIP, "Навигационный пси-маяк", { ct:"equipment", lc:1, r:4, desc:"Каждый на борту: +1 к Low Spirit, трейт Heretech (Psykers). Развёрнутый в системе: +15 к Navigation (Warp) к полёту в эту систему для знающих тональность вопля. Богохульство против Омниссии." }),
  // Боеголовки торпед
  E(SNAR_WAR, "Боеголовка: Плазменная", { ct:"ammo", lc:1, r:1, desc:"SPD 10, Урон 2d10+14, Крит 10, Дальн. 60. Terminal Penetration (3)." }),
  E(SNAR_WAR, "Боеголовка: Абордажная", { ct:"ammo", lc:1, r:2, desc:"SPD 10, Урон 2d10+11, Дальн. 60. Terminal Penetration (2)." }),
  E(SNAR_WAR, "Боеголовка: Мельта", { ct:"ammo", lc:1, r:3, desc:"SPD 10, Урон 2d10+15, Крит 9+, Дальн. 60. Terminal Penetration (4)." }),
  E(SNAR_WAR, "Боеголовка: Вирусная", { ct:"ammo", lc:1, r:4, desc:"SPD 10, Урон 2d10+10, Дальн. 60. Terminal Penetration (1)." }),
  E(SNAR_WAR, "Боеголовка: Вортексная", { ct:"ammo", lc:1, r:5, desc:"SPD 10, Урон 2d10+5, Крит 6+, Дальн. 60. Terminal Penetration (5)." }),
  E(SNAR_WAR, "Боеголовка: Атомная", { ct:"ammo", lc:1, r:4, desc:"SPD 10, Урон (1d10+5)×6, Крит 7+, Дальн. 60. Хранение требует ритуалов; при Повреждении аппаратов — уничтожается. Tech-Use(−10): провал → корабль получает половину урона." }),
  E(SNAR_WAR, "Боеголовка: Кровопийца (друкхари)", { ct:"ammo", lc:1, r:5, desc:"SPD 10, Дальн. 60. Цель: −2 SPD и −30 к увеличению скорости за каждую попавшую торпеду; снять — экстренным ремонтом (1 +1 за 2 СУ)." }),
  E(SNAR_WAR, "Боеголовка: Звуковая (эльдар)", { ct:"ammo", lc:1, r:5, desc:"SPD 10, Урон 2d10+5, Крит 10+, Дальн. 20. Terminal Penetration (5); урон игнорирует броню." }),
  E(SNAR_WAR, "Боеголовка: Взломщик", { ct:"ammo", lc:1, r:4, desc:"SPD 10, Дальн. 60. 1d5 (+1 за СУ) случайных систем выводятся из строя хакерской атакой." }),
  // Системы наведения
  E(SNAR_NAV, "Система наведения: Стандартная", { ct:"equipment", lc:0, r:0, desc:"TR 20. Идёт в комплекте с боеголовкой; не меняет правила залпа." }),
  E(SNAR_NAV, "Система наведения: Самонаводящаяся", { ct:"equipment", lc:0, r:2, desc:"TR 30. Обходит большую часть защиты целевого судна." }),
  E(SNAR_NAV, "Система наведения: Быстрогорящая", { ct:"equipment", lc:0, r:1, desc:"TR 15. Форсированный двигатель — выше скорость." }),
  E(SNAR_NAV, "Система наведения: Управляемая", { ct:"equipment", lc:0, r:1, desc:"TR 20. Управляемая система." }),
  // Мистическое
  E(SNAR_MYST, "Навигаторские карты варп-маршрутов", { ct:"equipment", lc:0, r:1, desc:"Навигатор автоматически читает карты своего дома; чужие — взлом FL (Navigators) −30. Дают +10 (Good +20 чтение / Best +20 чтение и прокладка) к Чтению знамений и Прокладыванию курса. Лишний регион: +1 R. Карты варп-штормов: +1 R (лоялистам +3) и 1d5 Порчи при Порче <20." }),
  // Стационарное
  E(SNAR_STAT, "Двойной Трон", { ct:"equipment", lc:2, r:3, desc:"Обладатель Psyniscience ведёт корабль по правилам Навигатора. Poor: варп-навигация −10. Good: читает навигаторские карты как навигатор дома. Best: использует Psyniscience и Navigation (Warp) самого пленного навигатора." })
];
