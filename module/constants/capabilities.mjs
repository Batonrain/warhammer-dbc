...
export const CAPABILITIES = {
  ...
  "dualWield.core.allGunsBlazing": {
    label: "Делая две атаки короткой/длинной очередью по одной цели, та проходит тест на Подавление+0. Если обе атаки длинной очередью — Подавление−20. Смоделировано (wdbc-pb60): режим огня второй руки — свой независимый выбор в окне «Обе руки» (по умолчанию Одиночный, первую руку не копирует), а модификатор и кнопка теста Подавления цели считаются сами, когда обе атаки пары реально очереди.",
    source: "All Guns Blazing / Огонь из Всех Орудий",
    reader: "module/rules/dual-wield-talents.mjs — allGunsBlazingMod; module/sheets/attack-dialog.mjs + attack/dialog.mjs — режим огня второй руки; module/combat/attack.mjs + attack-card.mjs — кнопка теста Подавления"
  },
  ...
  "rangedCore.core.hairTrigger": {
    label: "РЕАЛИЗОВАНО ЧАСТИЧНО (16.09.2026, wdbc-1rno.27/.37 — Караул спроектирован и реализован с нуля, wdbc-1rno.27): «Раз в Раунд при выстреле из Караула (до броска) персонаж проходит Awareness(P)+0 vs Awareness(P)+0; при победе действует первым независимо от Ag, и выстрел считается Незримым. Не работает с метательным, спреями и видимыми дозвуковыми снарядами». Автоматизировано: гейт «раз в Раунд», допустимость оружия (не метательное/не Распыление — «видимые дозвуковые снаряды» программно не отличить, честно оставлено столу), пометка «следующий выстрел — Незримый» (тот же примитив, что Hidden Threat) и «действует первым независимо от Ag» (принудительный исход simultaneousActionWinner, минуя сравнение характеристик). НЕ автоматизирован сам встречный тест Awareness vs Awareness — тот же уровень доверия столу, что у Blindside/Из Слепой Зоны: кнопка в карточке фиксирует уже подтверждённую победу, не катает её сама.",
    source: "Hair Trigger / Палец на Спуске",
    reader: "module/rules/hair-trigger.mjs (isHairTriggerUnseenPending/markHairTriggerUnseenPending/consumeHairTriggerUnseenPending/hairTriggerAllowedWeapon), module/combat/overwatch.mjs (offerOverwatchShot — кнопка встречного теста, resolveOverwatchHairTriggerClick), module/combat/attack.mjs (hairTriggerFlag → unseen)"
  },
  ...
  "rangedCore.core.scanningAdvance": {
    label: "РЕАЛИЗОВАНО (16.09.2026, wdbc-1rno.27/.37): «Персонаж может провести Полудвижение и Караул как одно полное действие, а его сектор обстрела в Карауле увеличивается до 90°». Отдельный HUD-пункт «Сканирующее Продвижение» (2 ОД целиком, не 1+2) — отмечает Полудвижение (markMovedThisTurn, для Импульсного и т.п.) и объявляет Караул с потолком сектора 90° вместо 45°. ИНТЕРПРЕТАЦИЯ (честно зафиксирована, не проверялась отдельно): расширенный сектор даёт только ЭТА комбинированная декларация — обычный «Караул» без движения остаётся 45° даже при купленном Таланте, читая книжный текст как «сектор ЭТОГО совмещённого действия», а не общую надбавку Карaулу. Сам перевод токена (как и у любого другого действия Движения в системе) не автоматизирован — драг мышью, реаch-ring не показан (в отличие от declareHalfMove — его внутренний _showReachRing не экспортирован, честная мелкая недоделка).",
    source: "Scanning Advance / Сканирующее Продвижение",
    reader: "module/rules/overwatch.mjs (overwatchMaxArc/OVERWATCH_SCANNING_ADVANCE_MAX_ARC), module/combat/overwatch.mjs (overwatchMenuItems/declareOverwatch scanningAdvance)"
  },
  ...
  "rangedCore.core.vigilance": {
    label: "РЕАЛИЗОВАНО (16.09.2026, wdbc-1rno.27/.37): «При определении очерёдности одновременных действий (Задержка, Караул), если действие персонажа — стрельба, он может использовать P вместо А». Задержка как отдельное действие в системе не реализована (Караул — единственный потребитель); при Карауле реагирующая сторона сравнивается по max(Ag.total, P.total), а не по одной Ag — тот же принцип «берётся лучшая из разрешённых», что у rules/initiative.mjs::initiativeCharKey.",
    source: "Vigilance / Бдительность",
  ...
  "gift.khorne.bloodAnointed": {
    label: "Реализовано в основном (wdbc-1rno): не перегружающийся щит-дефлектор 1-44 от стрелковых атак/взрывов, если персонаж в предыдущий Ход не стрелял и был связан в рукопашной. Выдаётся тем же тактом конца Хода, что и Щит Праздности (module/combat/turn-state-shield.mjs::processTurnStateShieldsTurnEnd, bloodAnointedRating) — «не стрелял» читает rules/turn-flags.mjs::attackedThisTurn+weaponClass, «связан» — реальный Базовый/Глубокий контакт с враждебным токеном личного масштаба (combat/free-attack.mjs::enemyContactTokenDocs, та же геометрия, что у Свободной Атаки). «Только от стрелковых/взрывов» — щит несёт флаг turnStateShieldRangedOnly, combat/damage.mjs::_rollActiveShield вовсе не рассматривает такой щит против рукопашного попадания (не «не срабатывает» — не участвует в выборе). НЕ реализовано: альтернативный триггер «шёл в направлении к противнику» — движок не хранит позицию НАЧАЛА Хода отдельно от текущей, посчитать «стало ближе к врагу» нечем (персонаж, весь Ход шедший к врагу, но не дошедший до контакта, щита не получит — реальный пробел, не выдумываю тест на его закрытие); эскалация 1-44→1-88 «измазан кровью» — состояние нигде не отслеживается и книга не даёт для него ни триггера, ни числа для автоопределения.",
    source: "Дар Кхорн (Blood-Anointed)",
    reader: "module/combat/turn-state-shield.mjs::bloodAnointedRating/processTurnStateShieldsTurnEnd (выдача, читает combat/free-attack.mjs::enemyContactTokenDocs), module/combat/damage.mjs::_rollActiveShield (фильтр RANGED_ONLY_FLAG), такт — module/hooks.mjs::updateCombat"
  },
  ...
  "gift.khorne.bronzeMyrmidon": {
    label: "РЕАЛИЗОВАНО ЧАСТИЧНО (15.09.2026, wdbc-1rno.1): Трейт Machine(+½Cor.b) в Ярости — отдельная запись kind:trait/when.requireRage (wdbc-wyr3). Редирект попаданий: пока Трейт активен, «Сочленение / Шея» и «Глаз (Голова)» резолвятся дальше (AP, крит-таблица) как попадания в «Рука»/«Голова» — module/rules/bronze-myrmidon.mjs, подключено в module/combat/damage.mjs::applyDamageToActor. «Конечности» книга не уточняет какая — взят генерик «Рука» (по аналогии с уже принятыми в системе неопределённостями стороны). «Визор» как отдельное место попадания в системе не существует — по прямому указанию пользователя приравнен к уже существующей цели «Глаз (Голова)» (Избирательная атака). Честная цена: редирект узнаётся только когда защищающийся уже выбран (module/combat/damage.mjs, при применении урона) — в исходной карточке атаки (attack.mjs) метка ещё показывается нередиректированной, тот же компромисс, что у Compression. «Тот же приём применён к скакуну/технике при верховой/пилотируемой» — НЕ проверено/не реализовано (отдельная сущность, вне объёма этой миграции).",
    source: "Дар Кхорн (Bronze Myrmidon)",
    reader: "module/rules/bronze-myrmidon.mjs (hasActiveMachineTrait/redirectHitLocationForMachine), module/combat/damage.mjs (applyDamageToActor — редирект перед резолвом AP/крита)"
  },
  ...
  "gift.khorne.knightOfKhorne": {
    label: "Реализовано (wdbc-1rno): готовый Ритуал «Призыв Связанного Скакуна (Кхорн)» — простым 5-минутным ритуалом без теста призывает Джаггернаута из Бестиария сразу Миньоном без слота, с реальным тикающим сроком дестабилизации (1d10+2×W.b−Inf.b Раундов, растёт по ступеням Завесы), который стоит, пока чемпион верхом. Владычество против СВОЕГО скакуна — автопобеда без теста (та же isOwnArmiger, что у Оруженосца). Второй Ритуал того же Дара вселяет демона в уже имеющегося скакуна/технику чемпиона (без нового Актора): +8 AP ТОЛЬКО от стрелковых атак (module/combat/damage.mjs, читает !melee), доп. кубик урона на встроенное оружие ближнего боя скакуна/техники (бьёт по system.damage embedded-оружия) и доп. кубик урона Тарана (module/combat/vehicle.mjs::_resolveRam, складывается с бонусом за скорость, не заменяет его). НЕ реализовано: форма «Вселение» отдельно от «Истинной Формы» — книга не даёт им разной механики в контексте ЭТОГО Дара (намеренно, оба исхода — призыв в Истинную Форму); освобождение демона из скакуна/техники — книга не описывает его явно для этого Дара (в отличие от оружия Оруженосца, где прямо сказано «оружие уничтожается»), поэтому решение стола.",
    source: "Дар Кхорн (Knight of Khorne)",
    reader: "packs-src/mutations/Дары_Богов/Кхорн/Knight_of_Khorne___Рыцарь_Кхорна_qA0S0sVfQLF9EbF4.json (entries knightOfKhorne-ritual/-ritual-mount) + packs-src/rituals/Архетипа/Summon_the_Bound_Mount___Призыв_Связанно_KnightKhorneR1ax.json + Bind_the_Mount_into_a_Vehicle___Вселение_KnightKhMountS1a.json; module/apps/demon-mount.mjs::bindDemonMount, module/apps/demon-summon.mjs::spawnDemonOnScene ({startDestabilize}), module/rules/demon-destabilize.mjs + module/combat/demon-destabilize.mjs (тикающий срок), module/rules/mount.mjs (mountRangedApBonus/mountRamExtraDie — геттеры), module/combat/damage.mjs (+8 AP не-рукопашным), module/combat/vehicle.mjs::_resolveRam (доп. кубик Тарана)"
  },
  ...
  "gift.khorne.theHunter": {
    label: "Реализовано (wdbc-1rno) записью kind:\"script\" на этом же предмете (цена 1 Бесчестие + 2 ОД полного действия): гейт видимого псайкера (nearestVisiblePsyker), реальный призыв Гончей Плоти (module/combat/the-hunter.mjs::spawnHunterHound — метка+синхронизация инициативы «сразу после чемпиона»), подсказка «ближайший псайкер» в карточке (автоатак в системе нет ни у одного существа — атаку ведёт игрок/ГМ). «Возвращается в Варп» — кнопка сама появляется в карточке «Констатировать смерть», если добыча убита именно этой Гончей",
    source: "Дар Кхорн (The Hunter)",
    reader: "packs-src/mutations/Дары_Богов/Кхорн/The_Hunter___Загонщик_cSEite9LGCwihpKz.json (entry theHunter-summon) — исполняется module/apps/item-script.mjs::executeItemCode"
  },
  ...
  "gift.nurgle.knightOfNurgle": {
    label: "Реализовано (wdbc-1rno): готовый Ритуал «Призыв Связанного Скакуна (Нургл)» — простым 5-минутным ритуалом без теста призывает Паланкин Нургла из Бестиария сразу Миньоном без слота, с реальным тикающим сроком дестабилизации, который стоит, пока чемпион верхом. Владычество против СВОЕГО скакуна — автопобеда без теста. Второй Ритуал того же Дара вселяет демона в уже имеющегося скакуна/технику чемпиона: фиксированные +7 Ран (живой скакун) или +7 Структуры (техника) и книжная гарантия «автоматически проходит тесты на Трудный Ландшафт» — module/combat/mount.mjs::showMountTerrainDialog при этом флаге не открывает диалог и не кидает тест вовсе (не «авто-Успех сверху», а книжное «проходит»). НЕ реализовано: форма «Вселение» отдельно от «Истинной Формы» (намеренно, см. Кхорн); освобождение демона из скакуна/техники — книга не описывает его явно для этого Дара.",
  ...
  "gift.nurgle.maggotParasite": {
    label: "ПЕРЕОЦЕНКА (wdbc-ux8a, 15.09.2026): раньше числилась целиком блокированной — «нет ни одного прецедента переноса сознания между Actor-документами». Прецедент появился (module/rules/actor-control.mjs + module/apps/actor-control.mjs, построен под Volunteer Actor) — часть находки реализована. Кнопка на предмете (module/apps/maggot-parasite.mjs::becomeParasiteHost, шаблон mutation.hbs) — разовое превращение S/T/A→10, Раны.max→7 (module/rules/maggot-parasite.mjs::parasiteHostUpdate, прямая правка редактируемой базы характеристик/Ран — в Конструкторе нет операции «установить абсолютное значение» для характеристик, только add/subtract). Размер(−2) — БЕЗ кода: тот же Mechanics-приём, что у Servoskull (kind:\"trait\", sourceUuid на компендиумный Size(X), rating:-2), запись живёт прямо на предмете «Опарыш-Паразит» в pack JSON. При смерти хоста (крит-эффект утверждает смерть, combat/damage.mjs::textAssertsDeath) — кнопка «Опарыш выскакивает» вместо «Констатировать смерть» (тот же приём слота, что уже даёт Volunteer Actor/Поцелуй Мимика, но гейт по capability ЖЕРТВЫ, не атакующего). Захват новой цели (game.user.targets, module/apps/maggot-parasite.mjs::captureNewHost) — ПОЛНАЯ миграция: предмет Дара физически переносится (createEmbeddedDocuments/deleteEmbeddedDocuments), Foundry-владение — через уже готовый relay (apps/actor-control.mjs), активный лист игрока (game.user.character) — Foundry v13 подтверждено: это поле НЕ GM-only (в отличие от Actor.ownership) — сам игрок-носитель или ГМ меняют прямым вызовом, relay не нужен. Старое тело не умирает мгновенно — 7ч worldTime-таймер (тот же такт, что Cast Out of Death/хирургия мононити), затем автоматическая смерть (setDeceased). ДОРАБОТАНО (wdbc-ux8a, второй заход, полный текст Трейта Parasite от пользователя): захват теперь идёт ЧЕРЕЗ общий механизм module/apps/parasite-trait.mjs (контакт/длительность/срыв — Состояние parasiticContact, кнопка «Опарыш выскакивает» в damage.mjs теперь начинает контакт, не мгновенный захват), captureNewHost вызывается из completeInfection по достижении счётчика 0. «Смертоносное Природное Оружие (Cor.b)» — РЕАЛИЗОВАНО: новое свойство оружия module/constants/weapon-properties.mjs::deadlyNaturalCorB (живой +Cor.b И к урону, И к Пробитию — module/combat/attack.mjs, тот же слот, что уже даёт Tainted урону, отдельный флаг), выдано новым природным оружием «Parasitic Bite/Паразитический Укус» (kind:\"integralAttack\", packs-src/weapons/Интегральные_атаки) — числа атаки (WS-тест, 1d10 Rending, вплотную) НЕ из книги (в тексте их нет), выбраны по аналогии с другими природными атаками сессии (Devourer/Tail), решение пользователя. «Не может быть выцелен Избирательной атакой» — РЕАЛИЗОВАНО: module/sheets/attack-dialog.mjs жёстко убирает все опции прицела у цели с этой capability (первый гейт по ЦЕЛИ, не по оружию, в этом файле).",
    source: "Дар Нургл (Maggot Parasite)",
    reader: "module/rules/maggot-parasite.mjs (parasiteHostUpdate/scheduleAbandonedHostDeath/isAbandonedHostDeathReady), module/apps/maggot-parasite.mjs (becomeParasiteHost/captureNewHost/checkAbandonedHostDeath), module/apps/parasite-trait.mjs (beginParasiticContact/completeInfection — маршрутизация сюда), module/combat/damage.mjs (кнопка), module/sheets/attack-dialog.mjs (иммунитет к Избирательной), module/combat/attack.mjs (deadlyNaturalCorB), module/rules/actor-control.mjs + module/apps/actor-control.mjs (владение/relay)"
  },
  ...
  "gift.nurgle.prophetOfGallerpox": {
    label: "РЕАЛИЗОВАНО ЧАСТИЧНО (15.09.2026, wdbc-1rno.1): полное действие «заразить машину» — кнопка на предмете, помечает Vehicle/Ship-актора system.gallerpoxInfected=true (нет отдельной сущности «подсистема корабля» — заражение живёт на самом акторе целиком). Пока флаг стоит, все не-Нурглиты на ТЕКУЩЕЙ СЦЕНЕ этого актора получают −30 к единственному реальному «тесту против яда» в системе — сопротивлению свойству оружия Toxic (module/rules/prophet-of-gallerpox.mjs, module/rules/sources.mjs::prophetOfGallerpox, scope \"poison\" в resolve-test.mjs). Честная цена приближения: «зона обслуживания машины» = «вся сцена этого актора» (не геометрический радиус — книга и не даёт формулы), снимается заражение вручную (нет своей команды «изгнать» ни у Дара, ни у системы). НЕ реализовано: штраф против БОЛЕЗНЕЙ (в системе вообще нет теста на сопротивление disease — type:\"disease\" это статичные данные без броска); удалённое вкл/выкл заражённой машины на расстоянии до 7км (у «включена/выключена» машина нет ни одного игрового последствия нигде в системе — km между сценами тоже нигде не считаются).",
    source: "Дар Нургл (Prophet of Gallerpox)",
    reader: "module/rules/prophet-of-gallerpox.mjs (isGallerpoxInfectedMachine/gallerpoxPoisonPenaltyApplies/actorsOnSameScene/gallerpoxPoisonPenaltyActive), module/rules/sources.mjs (registerRuleSource(\"prophetOfGallerpox\")), module/rules/resolve-test.mjs (scope \"poison\" в effectAppliesTo), module/hooks.mjs (_applyWeaponPropEffect — poisonTest: condition===\"poisoned\"), module/data/actor/vehicle.mjs и ship.mjs (system.gallerpoxInfected), пак-предмет: кнопка kind:\"script\" \"prophetOfGallerpox-infect\""
  },
  ...
  "gift.nurgle.theEqualizer": {
    label: "РЕАЛИЗОВАНО ПОЛНОСТЬЮ (15.09.2026, wdbc-1rno.1): атакующий с более высокой базовой Характеристикой перебрасывает Успехи — kind:\"reroll\", rerollWho:\"opponent\" на самой записи, module/rules/item-rules.mjs::opposedTargetRerollRules. Обе половины книги теперь покрыты: (1) атака — принудительное применение в sheets/attack/dialog.mjs (было и раньше); (2) противник как ИНИЦИАТОР встречного теста — принудительное применение теперь есть и в общем диалоге Навыка/Характеристики (sheets/actor-sheet.mjs::_runTest, гейт kind===\"opposed\"/\"opposedSafe\"). Прежний вердикт «Вид теста известен слишком поздно» оказался НЕВЕРНЫМ при перепроверке: ctx.targetActor уже был доступен до выбора кубика — реальным недостающим звеном была не архитектура сбора контекста, а отсутствие точки принудительного применения who:\"opponent\" вне attack-dialog.mjs (общий ruleRerollsHtml намеренно фильтрует такие перебросы из галочек диалога).",
    source: "Дар Нургл (The Equalizer)",
    reader: "module/rules/item-rules.mjs::opposedTargetRerollRules, module/rules/sources.mjs (registerRuleSource(\"opposedTarget\")), module/sheets/attack/dialog.mjs (форсировка на атаке), module/sheets/actor-sheet.mjs::_runTest (форсировка на встречном тесте Навыка/Характеристики)"
  },
  ...
  "gift.slaanesh.avatarOfGreed": {
    label: "Переброс атаки механизирован отдельной записью kind:\"reroll\" (rerollScope:all, rerollWho:self, БЕЗ цены) на этом же предмете — бесплатная галочка «Снаряжение лучше (решает ГМ)» в диалоге броска атаки, доступная всегда; книжное условие «цель экипирована лучше» не распознаётся автоматически, применимость решает игрок/ГМ, тот же принцип, что у остальных testMod-галочек. ПРОВЕРЕНО ЗАНОВО (13.09.2026, повторно 16.09.2026): в системе нет числового «рейтинга снаряжения» ни у одного актора — гасить нечего, условие целиком остаётся на суждение стола; по прямой просьбе пользователя (16.09.2026) подпись кнопки сокращена до «Снаряжение лучше».",
  ...
  "gift.slaanesh.hermaphrodite": {
    label: "+30 Обаяние механизировано отдельной записью kind:\"testMod\" (skillKey:charm) на этом же предмете. ПРОВЕРЕНО ЗАНОВО (13.09.2026, повторно 16.09.2026): остаток — «доступ к обычно иммунным целям» и «возврат утраченного либидо» — гасить программно нечего: в системе НЕТ иммунитета к соблазнению у Механикум/Астартес вообще (races.mjs не кодирует такую механику, Обаяние против них уже работает как против любого другого), и нет счётчика/состояния «либидо потеряно» ни у одного актора. Обе половины книги обещают снять ограничение, которого движок и так не накладывает — тот же класс находки, что Ever-Youthful/Omniglot этого же тикета. Перепроверка 16.09.2026 на новых примитивах сессии — ничего не меняют.",
  ...
  "gift.slaanesh.senseOfLust": {
    label: "Доп. чувство: засекает сексуальные эмоции в радиусе Cor.b км с направлением/природой, особенно чётко — влечение к самому чемпиону. Оставлен честной заглушкой по прямому указанию пользователя (сессия 12.09.2026, кластер Тзинч/по аналогии — тот же вывод подтверждён 13.09.2026, и ЕЩЁ РАЗ 15.09.2026 после перепроверки на новых примитивах сессии — actor-control.mjs/Cor.b-Пробитие/parasite-trait/condition-ticks generic-хук ничего не меняют, т.к. в системе физически нет данных «сексуальное влечение»/«нашёл бы привлекательным» ни у одного актора — не пробел в канале, пробел в самих данных): ни цены, ни триггера, ни числового порога в тексте нет — это чистый нарративный детектор без единого механического последствия где-либо в системе (никто не читает «нашёл бы персонажа привлекательным» программно), решение целиком за столом.",
  ...
  "gift.tzeentch.falseWitness": {
    label: "ПОДТВЕРЖДЕНО ЧЕСТНОЙ ЗАГЛУШКОЙ (12.09.2026, повторно 13.09.2026, повторно 15.09.2026): автослышит и распознаёт любую ложь (не честное заблуждение) в радиусе Cor м — «распознаёт ложь» движку в принципе негде проверить, правда/ложь реплики целиком в голове ГМа, данных об этом в системе нет и не может появиться. Единственный формально механический кусочек — плата 1 Очко Бесчестия за расширение радиуса до Cor км до конца сцены (реальный такт есть, apps/game-session.mjs) — но её автоматизация не имела бы функционального потребителя: ни одна часть системы не читает «в радиусе ли цель», список услышанной лжи не формируется — декоративная кнопка без механики за ней, решено не тащить. Перепроверка 15.09.2026 на новых примитивах сессии (actor-control.mjs/Cor.b-Пробитие/parasite-trait/condition-ticks generic-хук) — ничего не меняют, пробел в данных («истинность высказывания»), не в канале.",
  ...
  "gift.tzeentch.infernalArmiger": {
    ...
    reader: "packs-src/mutations/Дары_Богов/Тзинч/Infernal_Armiger___Инфернальный_Оруженос_dhnszZq38C0nRzNH.json (entries infernalArmiger-ritual/-ritual-weapon) + packs-src/rituals/Архетипа/Summon_the_Bound_Armiger___Призыв_Связан_ArmigerTzeenQ4d2.json + Bind_the_Armiger_into_a_Weapon___Вселени_ArmigerTzWeapQ4d.json + packs-src/traits/Thinner_Veil__Master_s_Corruption____Тон_ArmigerVeilThin1.json"
  },
  "gift.tzeentch.knightOfTzeentch": {
    label: "Реализовано (wdbc-1rno): готовый Ритуал «Призыв Связанного Скакуна (Тзинч)» — простым 5-минутным ритуалом без теста призывает Диск Тзинча из Бестиария сразу Миньоном без слота, с реальным тикающим сроком дестабилизации, который стоит, пока чемпион верхом. Владычество против СВОЕГО скакуна — автопобеда без теста. Второй Ритуал того же Дара вселяет демона в уже имеющегося скакуна/технику чемпиона: И скакуну/технике, И всаднику выдаётся предмет-forcefield (rating 1-50, overloadThreshold:0 — «не перегружается», тот же приём, что у готовых Даров с похожей прозой). НЕ реализовано: форма «Вселение» отдельно от «Истинной Формы» (намеренно, см. Кхорн); освобождение демона из скакуна/техники — книга не описывает его явно для этого Дара (щиты, выданные вселением, тоже не снимаются автоматически).",
    source: "Дар Тзинч (Knight of Tzeentch)",
    reader: "packs-src/mutations/Дары_Богов/Тзинч/Knight_of_Tzeentch___Рыцарь_Тзинча_de3gvGhKnV1szIK0.json (entries knightOfTzeentch-ritual/-ritual-mount) + packs-src/rituals/Архетипа/Summon_the_Bound_Mount___Призыв_Связанно_KnightTzeenR4d2x.json + Bind_the_Mount_into_a_Vehicle___Вселение_KnightTzMountS4d.json; module/apps/demon-mount.mjs::bindDemonMount (createEmbeddedDocuments forcefield на mount+riderActor)"
  },
  ...
  "mutation.pureForm": {
    ...
    reader: "module/rules/mutation-suppression.mjs (setMutationsSuppressed), module/apps/effects.mjs (isItemActive читает flags.warhammer-dbc.suppressed), module/rules/wounds.mjs::woundLossUpdates (аварийный разрыв, 1d10 непогл. урона)"
  },
  ...
  "mutation.mistTransformation": {
    label: "РЕАЛИЗОВАНО ЧАСТИЧНО (reader ошибочно оставался пустым, тот же класс находки, что «Desiccated»/Pure Form, перепроверено 16.09.2026): выдача Трейтов Incorporeal+Flyer(A.b×2) механизирована отдельной записью kind:\"script\" (ручная кнопка на листе предмета) на этом же предмете: клонирует обе Черты из пака, дедуп-проверка, длительность Cor.b минут снимается АВТОМАТИЧЕСКИ (rules/temp-grant.mjs — worldTime-таймер, снятие на updateWorldTime/updateCombat, hooks.mjs). НЕ автоматизированы: частота «Cor.b раз/сутки, не чаще раза в 10−Cor.b Раундов» (throttle Конструктора не умеет формулу, тот же остаток, что у Spatial Instability, wdbc-1rno.8), радиус облака/неуязвимость/блокировка обзора, растворение до Cor.b союзников. wdbc-1rno — не проверено живьём в Foundry (мир не запущен на момент правки)",
    source: "Мутация: Mist Transformation (Общие мутации)",
    reader: "packs-src item mistTransformation-script-entry (kind:\"script\"), module/rules/temp-grant.mjs (worldTime-таймер снятия), module/hooks.mjs (updateWorldTime/updateCombat)"
  },
  ...
  "mutation.fruitOfFlesh": {
    label: "12 субмутаций (d10), раз в сутки. Реализованы кодом (kind:\"script\" fruitOfFlesh-activate/-eat на этом же предмете, module/apps/fruit-of-flesh.mjs, диспетчер по item.system.submutation.label): «0» Плод Исцеления — самоотчёт непоглощённого урона (реактивный перехват урона система не умеет, тот же принцип self-report, что Tireless Warrior), плод созревает 3 дня, съевший лечит min(2d10, вместимость). «1» ЭМИ — самоотчёт рейтинга (Haywire в этом движке мгновенный, без персистентного состояния «в поле», нечем гейтить автоматически), ЭМИ-граната Haywire(рейтинг); «не действует на самого персонажа» — не отдельный код, обычный бросок гранаты и так не задевает бросающего. «2-3» Дым — АВТОМАТИЧЕСКАЯ проверка (регион Трудного Ландшафта со свойством smoke под токеном, regions/difficult-terrain.mjs), радиус — самоотчётом (геометрия Region-фигур не читается напрямую), облако удаляется целиком; «завесовая бомба» как отдельный предмет не существует ни у одного оружия в packs-src — граната всегда обычная Дымовая. «4-5» Пламя — тушит Горение себе и радиусу Cor.b м, Flame-граната с рейтингом = максимум magnitude потушенного (condition-ticks.mjs::burningSourceDamage). «6» Оглушение/Беспомощность/Стазис — три РАЗНЫХ триггера (Оглушён/Беспомощен/в Стазисе), каждый снимает своё Состояние и даёт свою фиксированную гранату (Оглушающая/Паутинная/Стазис-Бомба); Стазис — НОВОЕ Состояние (module/constants/conditions.mjs::stasis, заведено по прямому запросу пользователя на основе этой находки): абсолютный иммунитет к урону (TOTAL_IMMUNITY-гейт в combat/damage.mjs::applyDamageToActor, до брони/Ран/Крит-эффектов), «считается Беспомощным» — чисто описательная связь (в реестре Состояний нет кросс-влияния друг на друга), «невозможно сдвинуть с места» — честно не автоматизировано (в системе нет ни одной механики принудительного перемещения вообще). «Не действует на Слаанешитов» (взрыв гранаты) — честно не автоматизировано, в системе нет общего примитива «AoE не действует на покровительство X». «7» Яд/Радиация — снимает Отравление/Радиацию себе и радиусу, граната Blast+Rad(десятки снятой дозы)+Toxic. Rad-рейтинг в движке НЕ масштабирует эффект (у свойства \"rad\" нет testPerRating ни у одного оружия в системе, та же картина у штатной гранаты Rad компендиума), число чисто описательное. «9» Заточение Силы (только tzeentch) — половина реализована: провал встречного теста на W — самоотчёт-выбор (как и везде в этой находке); цель ищется тем же cross-actor приёмом, что уже даёт rules/psychic-sustain-target.mjs (isSustained && sustainedTargetUuid===actor.uuid); замок — флаг fruitOfFleshLockUuid НА САМОЙ психосиле, блокирует чекбокс «Развеять» в sheets/tabs/psychic.mjs::.psy-sustain-cb (то же место, что уже блокирует Саркофаг Дредноута), разблокировка — по факту уничтожения Плода (fromUuid не резолвится, без отдельного хука на удаление). «...или покинет радиус поддержания» — заведён bd wdbc-efyl (общего понятия «радиус поддержания» нет ни у одной психосилы в системе), подключить сюда когда будет готово. Демонический полу-текст («атака Одержимостью» → «запереть демона») — заведён bd wdbc-q267 (ни «атака Одержимостью» как тестируемое событие, ни «Демонхост» не существуют в системе вообще — целая боевая подсистема, не мелкий примитив), тоже на будущее. «10» Осколки (только khorne) — рейтинг Калечащего РЕАЛЬНО хранится (system.crippledWounds[].rating, wdbc-1rno-совместимая находка вопреки первоначальной ошибочной разведке — не архитектурный пробел): растворяет ВСЕ осколки/застрявшие стрелы себе и радиусу Cor.b м (не только самый мощный), Piercing считается Crippling(3) книжной формулой, Фраг-граната получает Tainted+Crippling(максимум снятого). «11» Тройной Плод — «Бросьте 3 раза без Inf.b» реализовано ОБЩИМ механизмом субмутаций (module/rules/submutations.mjs::multiRollCount/multiRollResults, module/apps/submutations.mjs, module/data/item/mutation.mjs::submutation.multi — новое поле схемы), не точечным кодом для одной находки: точно такая же строка есть у Wings/Крылья «12 — Многокрылый» (проверено пользователем — исправила свою более раннюю ошибочную оценку «единственная во всём тексте»). Дубликаты/попадание на саму multi-roll строку схлопываются в список короче трёх (книжная фраза «или меньше, если были дубликаты» — прямая цитата). Игрок выбирает, какой из до-трёх результатов активировать сейчас (каждый — отдельно, книга не даёт «все три одним кликом»); общий суточный лимит уже даёт framework (одна кнопка kind:\"script\" на весь предмет) — досочинять не пришлось. Остаётся: «8» Пси-атака с уроном (архитектурно блокирована — нет модели «одна атака — общий список жертв» для постфактум-нивелирования у ДРУГИХ целей, и нет примитива «скопировать эффект произвольной психосилы на предмет») — честная заглушка, activateFruitOfFlesh предупреждает для её подписи субмутации.",
    source: "Мутация: Fruit of Flesh (Общие мутации)",
    reader: "module/apps/fruit-of-flesh.mjs::activateFruitOfFlesh/eatHealFruit (диспетчер по submutation.label), module/rules/fruit-of-flesh.mjs (арифметика)"
  },
  ...
  "mutation.boneless": {
    label: "+10 A механизировано отдельной записью kind:\"characteristic\" на этом же предмете — capability покрывает ТОЛЬКО остаток: ½ I(Cr) Dmg (окр.▲) до Поглощения, иммунитет к Критическим Эффектам переломов; без опоры в жёстком доспехе — Athletics+0 или Amorphous+Crawler (−20 S, −2 Unnatural S, +30 Карабканье); опционально Quadruped(1) с модифицированной бронёй — не смоделированы. По решению пользователя (16.09.2026) весь остаток отложен отдельным тикетом wdbc-1rno.9 (5 независимых кусков, часть требует нового примитива движка — множитель урона по подвиду до Поглощения).",
  ...
  "mutation.majesticHorns": {
    label: "+20 социальные тесты с Хаоситами/Орками механизировано отдельной записью kind:\"testMod\" на этом же предмете. УТОЧНЕНО 16.09.2026 (reader ошибочно не отражал реальный охват — тот же класс, что Pure Form/Mist Transformation): субмутации 4-5 (Пронзающие Рога → Deadly Natural Weapons ½Cor.b(окр.▼)) и 6 (Рога Демонетки, slaanesh → +10 соц./Командование) УЖЕ реализованы отдельными записями. Субмутация 8 (Рога Кровожада, khorne) реализована ЧАСТИЧНО — база Natural Weapons/Deadly есть, но БЕЗ Concussive(0) и условного Flame(3d10) в Ярости. Capability покрывает остаток: субмутация 1 (щит-дефлектор 1-50 только в голову, не в глаза — нет kind:\"shield\" в Конструкторе вообще), 2-3 (+Cor.b AP против I(Cr) именно в голову — есть только AP-бонус по подвиду без привязки к локации), 7 (обломанный рог в ране → авто-провал T с 3 Провалами, регенерация 3ч — нет паттерна «встроенный в цель объект» нигде в коде), 9 (броня глаз/минус авточувства — нет armourLocation:\"eye\" в Конструкторе вообще), 10 (Корона Рогов — полный альт.профиль атаки, технически реализуемо через kind:\"integralAttack\" как Toothy Tongue, но не сделано), и довершение 8 (weaponProp Concussive+условный Flame). Вынесено отдельным тикетом wdbc-1rno.10 по решению пользователя.",
  ...
  "mutation.soulSeer": {
    ...
    reader: "module/apps/soul-seer.mjs::useSoulSeer, module/rules/soul-seer.mjs (soulSeerCategory)"
  },
  ...
  "mutation.organOfChaos": {
    label: "Реализовано (wdbc-1rno): книга не даёт формулы (характеристику/малую способность решает ГМ на КАЖДУЮ выдачу, не формула) — кнопка «Настроить орган» на конкретном экземпляре предмета у конкретного персонажа: диалог выбора Характеристики пишет реальную запись kind:\"characteristic\" (field:\"bonus\", value:1 — Unnatural X(+1), тот же вид записи, что и везде в системе), малая способность (свободный текст, книга не даёт для неё общей формулы) — в system.notes предмета. Тот же приём выбора-при-выдаче, что apps/hand-of-khorne.mjs::setApEntry (найти-или-создать запись по фиксированному id).",
    source: "Мутация: Organ of Chaos (Общие мутации)",
    reader: "module/apps/organ-of-chaos.mjs::useOrganOfChaos/organOfChaosButtonHtml, sheets/item-sheet.mjs (.organ-of-chaos-btn), templates/item/parts/mutation.hbs"
  },
  ...
  "mutation.wrappedInChaos": {
    label: "10 субмутаций (d10). Группа A реализована кодом: «4-5» Дымовая Завеса — kind:\"script\" wrappedInChaos-activate, шаблон Smoke(5) (тот же примитив, что уже даёт Fruit of Flesh/Плод Плоти); Cor 30+ «видит сквозь» — честно не автоматизировано (в системе нет движка видимости вообще). «7» Мухи — НЕ кнопка на этом предмете (эффект бьёт АТАКУЮЩЕГО, не владельца): реализована прямым чтением цели в module/sheets/attack-dialog.mjs (тот же приём, что уже даёт «Цель Повалена»/«Цель Оглушена» — статический badge + строка в thresholdParts), базовый штраф −5 растёт по тиру Ран цели (healthy/light/heavy/dying) до −20, эскалация при Избирательной атаке (до −40) читает живой чекбокс f.aimPenalty. «Полагающиеся на зрение» не сужается — применяется ко всем атакам. «10» Осквернённый Клинок — 1 непоглощ. урона себе → выбранному рукопашному оружию свойство Tainted; ДЛИТЕЛЬНОСТЬ ОКРУГЛЕНА: книга даёт «12 Раундов», реализовано как «до конца боя» (module/combat/wrapped-in-chaos.mjs::clearTaintedBladeBuffs на deleteCombat, тот же приём, что Blood Flame/Кровавое Пламя) — точного счётчика Раундов для временных weaponProp-грантов в системе нет ни у одной находки. «Клинковое» не сужается — система не классифицирует оружие по форме клинка, выбор любого рукопашного оружия свободный. «9» Рассечение Реальности — реализована кодом ПОСЛЕ переоценки (изначально числилась в Группе B «несёт готовый примитив system.incomingDamageReduction», при реализации выяснилось — поле заполняется ТОЛЬКО наркотиками, rules/character.mjs, жёстко зашито под drug.specialEffects.reduceDamageOnHit; по прямому решению пользователя примитив ОБОБЩЁН, не захардкожен под находку): module/rules/wrapped-in-chaos.mjs::realityRendingPenalty — чистая пересчётная функция (тот же паттерн межакторного правила, что psychic-sustain-target.mjs — без импортов, сканирует game.actors живьём в момент вычисления, ничего не хранит на цели), читается ЖИВЬЁМ в момент урона (combat/damage.mjs, рядом с Решимостью сражаться/Лишь Свет, отдельная строка в карточке; wdbc-bjy1.3 — раньше запекалась в system.incomingDamageReduction при пересчёте актора и не следила за движением токенов); действует на всех носителей токена владельца в радиусе 3м — книга даёт «+3 ко всему входящему урону», реализовано как −3 в incomingDamageReduction (это поле ВЫЧИТАЕТСЯ из урона в combat/damage.mjs, отрицательное значение = урон выше на ту же величину), суммируется от нескольких владельцев; «стабилизировать часть пространства, исключив до W.b союзников» — кнопка-диалог на предмете (module/apps/wrapped-in-chaos.mjs::useStabilizeRealityRending/promptExcluded, список UUID акторов во flags.warhammer-dbc.realityRendingExcluded, обрезан по текущему W.b владельца). НАЙДЕН И ИСПРАВЛЕН импорт-цикл: rules/wrapped-in-chaos.mjs НЕ импортирует tokenDocDistance из regions/auras.mjs (тот тащит apps/mechanics.mjs → regions/difficult-terrain.mjs, где DifficultTerrainBehaviorType расширяет foundry.data.regionBehaviors.RegionBehaviorType на верхнем уровне модуля — ReferenceError при импорте без Foundry-стаба) — формула продублирована локально, чтобы rules/character.mjs остался безопасен для тестов на «голых» объектах без стаба. «1» Тень — ПЕРВЫЙ примитив телепортации токена в проекте (по прямому запросу пользователя, не оставлено текстом): интерактивное размещение точки через canvas.regions.placeRegion (тот же клик-механизм, что уже даёт placeSmokeZone) — временный маркер-регион, координаты снимаются, регион тут же удаляется; проверка «в темноте» (свойство ландшафта \"dark\"), дистанция ≤10м от текущей позиции (tokenCenter/pxPerMeter), «раз в Раунд» — свой троттлинг (rules/cooldown.mjs, isCapabilityAvailable/markCapabilityUsed unit:\"round\"), независимый от общего Constructor-throttle этого диспетчера (тот один на все 10 субмутаций). «6» Сладкий Туман — газ радиусом 3м, СНИМОК в момент активации (не длящаяся зона — тот же снимок-приём, что у Мух/Жара Гнева по духу этого тикета); НОВОЕ Состояние sweetMist (module/constants/conditions.mjs, по прямому указанию пользователя): «−10 W (до −20 vs Charm), 3 часа после вдыхания» — момент истечения хранится доп. полем sweetMistExpiresAt (worldTime, тот же приём ручной надстройки сверх счётчика, что уже есть у burningSourceDamage/burningGraceRounds — в реестре Состояний нет 4-го типа counter под «часы»), снимается module/apps/wrapped-in-chaos.mjs::sweepSweetMistExpiry — зовётся из ТОГО ЖЕ Hooks.on(\"updateWorldTime\",…) в warhammer-dbc.mjs, что уже двигает виджет Календаря (по прямому указанию пользователя — «привязать к виджету Календаря»), не отдельным новым хуком. Респиратор/Противогаз — НОВЫЙ предикат module/rules/predicates.mjs::wearsGasProtection (Головной слот, gearCategory:\"head\", по имени — оба предмета не несут отдельного признака иммунитета к газу в системных полях, только текст/testMod). Сам штраф −10/−20 на тесты W — ОПИСАТЕЛЬНЫЙ (как у большинства Состояний книги, poisoned/dazed и т.п.): в системе нет центральной точки резолва тестов W, куда его можно подключить одной правкой (в отличие от Атаки — там уже есть attack-dialog.mjs). ПЕРЕОЦЕНКА «2-3» Фантомные Копии: изначально числилась архитектурно блокированной («нет направленного модификатора атакующий↔защитник для Защиты — только для Атаки, как у Мух»), но при разборе выяснилось — module/combat/defense.mjs уже резолвит attackerUuid в attackerActor ДО построения профиля теста (тот же путь, что уже даёт Разница Размеров Парированию, hand-of-khorne.mjs) — перенесена в реализованные: штраф −10 Уклонению защитника от рукопашной атаки владельца (module/combat/defense.mjs::_performDodge, modParts «Фантомные Копии атакующего»), бонус +20 на тесты Финта владельцу (module/combat/techniques.mjs::_showContestDialog, гейт по имени техники techDef.label===\"Финт\", тот же приём, что уже даёт Танец Обмана). Бонус к тестам Вольтов — честный блок: «Вольт» в системе НЕ отдельный бросок с порогом (module/combat/recoil.mjs — флаг disengageActive, навешивается ПОСЛЕ уже состоявшегося успешного Уклонения), нет к чему прибавлять число. «8» Жар Гнева (половина: штраф атакам против него/союзников) — реализована тем же приёмом, что Мухи (module/sheets/attack-dialog.mjs, wrathHeatMod, только рукопашные атаки), радиус 3м до союзника-держателя — тот же скан game.actors/tokenDocDistance, что у Рассечения Реальности; гейт книги «в Ярости ИЛИ связан рукопашной» сведён к «действует всегда при самой рукопашной атаке» (решение пользователя — «связан рукопашной» тавтологично верно в момент самой атаки). Плавка краски/испарение жидкостей в радиусе 1м от «8» — косметика, не автоматизирована. Находка «2-3»/«8» закрыта полностью — субмутации 1–10 Wrapped in Chaos все разобраны, ничего в очереди не осталось.",
    source: "Мутация: Wrapped in Chaos (Общие мутации)",
    reader: "module/apps/wrapped-in-chaos.mjs::activateWrappedInChaos (Дымовая Завеса, Осквернённый Клинок, Тень, Сладкий Туман)/sweepSweetMistExpiry/useStabilizeRealityRending, module/combat/wrapped-in-chaos.mjs::clearTaintedBladeBuffs, module/sheets/attack-dialog.mjs (targetFlies/fliesMod/fliesAimedMod — Мухи; wrathHeatMod — Жар Гнева), module/combat/defense.mjs::_performDodge (phantomCopiesDodgePenalty — Фантомные Копии), module/combat/techniques.mjs::_showContestDialog (phantomCopiesFeintBonus — Финт), module/rules/wrapped-in-chaos.mjs::realityRendingPenalty/hasPhantomCopies/phantomCopiesDodgePenalty/phantomCopiesFeintBonus/wrathHeatAttackPenalty + арифметика прочих, module/combat/damage.mjs (Рассечение Реальности — живьём в момент урона), module/rules/predicates.mjs::wearsGasProtection"
  },
  ...
  "runicWeave.breakwater": {
    label: "Раз/Раунд заставляет псайкера развеять/отменить на себе Телекинез или принудительное перемещение — Телекинез не выделен отдельной проверяемой силой в коде, не автоматизировано",
  ...
  "runicWeave.reaper": {
    label: "Выжигание Души на ×2 максимума Ран восстанавливает W.b Ран/урона цели псайкеру; выгорает после 3 применений — Выжигание Души (hooks.mjs _resolveSoulBurn) не ведёт счётчик применений/лечение кастера, не автоматизировано",
  ...
  "trait.parasite": {
    label: "Реализовано частично (wdbc-ux8a). «Паразит должен коснуться жертвы и сохранять контакт как минимум 1d5 Ходов +1 Ход за каждый уровень брони в точке контакта... Успешно завершив заражение, получает полный контроль над телом и разумом хоста — действует в его Инициативу, использует его I/P/W, лучший из WS/BS... не может использовать психосилы, Чудеса Веры, Warp-Gifted». Контакт/длительность/срыв — module/apps/parasite-trait.mjs::beginParasiticContact (кнопка на предмете-Трейте, цель — game.user.targets, 1d5 + ручная поправка на броню/Pen — нет автосканера брони именно точки контакта)/tearOffParasite (1d5 непоглощ. Rending в торс) — Состояние parasiticContact (module/constants/conditions.mjs), счётчик тикает генерик-циклом combat/condition-ticks.mjs, спец-хук на 0 зовёт completeInfection, который маршрутизирует: Опарыш-Паразит/Maggot Parasite (свой текст переопределяет финал на полное поглощение, module/apps/maggot-parasite.mjs) vs общий фьюжн (флаг possessedByParasiteUuid). Числовая часть фьюжна — module/rules/parasite-trait.mjs::fuseParasiteCharacteristic (цикл характеристик character.mjs — до Бонусов/Навыков/Здравомыслия, wdbc-bjy1.4) — Int/Per/WP паразита вместе с Бонусами, applyParasiteFusion (хвост) — Инициатива, WS/BS — max(хост, паразит); блок собственных психосил — module/sheets/tabs/psychic.mjs::showManifestDialog (тот же гейт-приём, что mimicWireBlocksPowers). НЕ автоматизировано (честная граница, отмечено и в notes самого предмета): перенос Навыков/Талантов хосту (нет канала временной выдачи чужих без физического клонирования предметов), блок Чудес Веры/Warp-Gifted (нет единой точки активации, как у психосил/техночудес), редирект неизбирательных атак в хост (требовал бы различимого отдельного токена паразита ВНУТРИ хоста — в текущей модели слияния его нет).",
    source: "Parasite / Паразит (Трейт — общий, packs-src/traits/Элитные_архетипы/Медуза, использует также Опарыш-Паразит/Maggot Parasite и архетип Медузы)",
    reader: "module/rules/parasite-trait.mjs (флаги/applyParasiteFusion), module/apps/parasite-trait.mjs (beginParasiticContact/tearOffParasite/completeInfection/endParasiticPossession), module/combat/condition-ticks.mjs (спец-хук на 0), module/rules/character.mjs (хвост prepareCharacterDerived), module/sheets/tabs/psychic.mjs (блок манифестации)"
  },
  "talent.kingsPlate": {
  ...
  "talent.volunteerActor": {
    label: "Реализован кодом (wdbc-ux8a, Аэльдари — Атхистаур/Мимик). «Если рукопашная атака по правилам Silent Elimination с модификацией «Поцелуй Мимика» ДОЛЖНА убить цель, персонаж может вместо этого оставить её с 1 раной и ввести мононить... лишается речи и движения... может управлять целью». Триггер «атака должна убить» НЕ автодетектируется (в системе нет сигнала «атака смертельна» — смерть текстовая, только combat/crit-effect-parser.mjs::textAssertsDeath даёт кнопку, а не автоматический факт), равно как Silent Elimination/«врасплох со спины» — нет детектора «атака из скрытности». Кнопка «Поцелуй Мимика» (module/combat/damage.mjs, заменяет «Констатировать смерть») появляется, когда МАШИНОЧИТАЕМО сошлись 3 условия: крит-эффект утверждает смерть (textAssertsDeath), атакующий несёт этот Талант, удар нанесён именно оружием «Поцелуй Арлекина» (module/rules/volunteer-actor.mjs::isHarlequinsKissItem, по имени) — Silent Elimination и «со спины» подтверждает стол самим кликом. Клик (module/apps/volunteer-actor.mjs::captureWithMimicWire): Раны цели → 1, накладывает НОВОЕ Состояние mimicWire (module/constants/conditions.mjs) — «лишается речи и движения» ОПИСАТЕЛЬНО (как у helpless, нет общего isHelpless()-гейта в системе), доп. поле mimicWireBlocksPowers — «за 10 сек/1м можно лишить психосил/техночудес» (машиночитаемо блокирует манифестацию через module/sheets/tabs/psychic.mjs, тот же приём, что highSorceryManifestBlocked у Perfect Sorcerer), устанавливает контроль через ОБЩИЙ НОВЫЙ примитив module/rules/actor-control.mjs (по прямому запросу пользователя — «контроль чужого токена» переиспользуем на будущие находки: Maggot Parasite/Опарыш-Паразит и т.п., НЕ используем на Exarch of the Eagle Pilots — там свой экипаж, не адверсариальный захват). Обрыв нити (кнопка «Оборвать нить») — смерть цели, реюз существующего deathButtonHtml/textAssertsDeath (не новый механизм смерти — в системе смерть всегда так). Хирургическое извлечение без вреда — 16ч worldTime-таймер (module/rules/volunteer-actor.mjs, тот же такт, что Cast Out of Death) + тест Medicae(I)−40 в конце — ПОДТВЕРЖДАЕТСЯ ВРУЧНУЮ кнопкой (система не гонит бросок Навыка сама, честная граница, как у остального тикета). Реальная передача Foundry-владения токеном (можно физически подвигать чужой токен) — необязательный довесок, relay через активного ГМ (Actor.ownership правит только GM, Foundry v13; тот же приём, что apps/demon-mount.mjs), best-effort — при отказе (нет активного ГМ) правило-флаг всё равно работает.",
    source: "Volunteer Actor / Доброволец Актёр (Аэльдари — Атхистаур/Мимик, packs-src/aeldari-talents/Элитные_архетипы)",
    reader: "module/rules/volunteer-actor.mjs (isHarlequinsKissItem/scheduleMimicWireSurgery/isMimicWireSurgeryReady), module/apps/volunteer-actor.mjs (captureWithMimicWire/cutMimicWire/startMimicWireSurgery/checkMimicWireSurgery/confirmMimicWireExtraction), module/combat/damage.mjs (кнопка вместо deathButtonHtml), module/rules/actor-control.mjs + module/apps/actor-control.mjs (общий примитив контроля)"
  },
