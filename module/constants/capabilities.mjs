...
export const CAPABILITIES = {
  ...
  "mutation.polymath": {
    label: "Полностью механизирована пятью отдельными записями на этом же предмете: +10 тесты Ремесла (kind:\"testMod\", skillKey:trade), +10 тесты Запретных знаний (kind:\"testMod\", skillKey:forbiddenLore — вкладка Мастерской Крафта уже требует именно эту группу для режима «Исследование», см. apps/craft-workshop.mjs::_reqs), плюс по одной записи kind:\"script\" с scriptTrigger:critSuccess на каждую из двух групп (+1d5 Усталости, напоминание сделать ещё тест — wdbc-1rno, новый примитив автозапуска по Крит.Успеху). Capability покрывает только нарративный остаток без чисел: определение дистанций/пропорций на глаз, точное воспроизведение чертежей по разобранному механизму",
