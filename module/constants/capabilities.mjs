...
export const CAPABILITIES = {
  ...
  // источника с таким текстом (не только Щупальца): было у gift.khorne.
  // livingWeapon и у флейвора «Опутанная Корнями»/дрюкхарийских биоимплантов,
  // ни один раньше не заводил capability. Актёрский уровень (не «этот конкретный
  // предмет в этой конкретной руке») — тот же компромисс детализации, что и у
  // остальных Возможностей без per-limb модели в системе.
  "combat.cannotBeDisarmed": {
    ...
    source: "Мутация: Tentacle, субмутация 4-5 «С Присосками» (Общие мутации); тот же текст встречается у gift.khorne.livingWeapon, «Опутанной Корнями» (Extra Arm) и биоимплантов дрюкхари, куда его тоже стоит завести при следующей правке этих записей",
