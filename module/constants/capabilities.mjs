...
export const CAPABILITIES = {
  ...
  "mutation.spatialInstability": {
    label: "Выдача Трейта Incorporeal механизирована отдельной записью kind:\"script\" (ручная кнопка «▶ Запустить» на листе предмета — не автозапуск) на этом же предмете: клонирует Incorporeal из пака Черт, дедуп-проверка от повторной выдачи. НЕ автоматизированы: частота «раз в 12−Cor.b ч» (нет формульного throttle), длительность Cor.b Раундов/добровольный возврат (снятие Черты — вручную, чат-напоминание при выдаче), 10 god-гейтнутых субмутаций. wdbc-1rno — не проверено живьём в Foundry (мир не запущен на момент правки)",
  ...
  "mutation.mistTransformation": {
    label: "Выдача Трейтов Incorporeal+Flyer(A.b×2) механизирована отдельной записью kind:\"script\" (ручная кнопка на листе предмета) на этом же предмете: клонирует обе Черты из пака, дедуп-проверка. НЕ автоматизированы: частота «Cor.b раз/сутки, не чаще раза в 10−Cor.b Раундов» (нет формульного throttle), радиус облака/неуязвимость/блокировка обзора, растворение до Cor.b союзников, длительность Cor.b минут (снятие Черт — вручную, чат-напоминание при выдаче). wdbc-1rno — не проверено живьём в Foundry (мир не запущен на момент правки)",
