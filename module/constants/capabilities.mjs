...
export const CAPABILITIES = {
  ...
  // остальных Возможностей без per-limb модели в системе: у Extra Arm это
  // безопасно (постоянный эффект руки с этой субмутацией, ничего не включает
  // и не выключает), а вот gift.khorne.livingWeapon НЕ подключён намеренно —
  // там иммунитет длится только «до конца боя/сцены» после полудействия+1
  // Бесчестия, а состояния «способность сейчас активна» система нигде не
  // хранит (ни для этого Дара, ни для родственных «до конца боя» тогглов) —
  // заведение здесь grantFlag'ом без условия сделало бы иммунитет ПОСТОЯННЫМ
  // от одного факта владения предметом, что RAW не подтверждает. Требует
  // сперва отдельной инфраструктуры «тоггл до конца боя/сцены» (см. план
  // wdbc-vyua, doombc-buff-choice-mechanic-plan), не просто одной строки here.
  "combat.cannotBeDisarmed": {
    ...
    source: "Мутация: Tentacle, субмутация 4-5 «С Присосками» (Общие мутации); субмутация 7 «Опутанная Корнями» (Extra Arm) подключена тем же капабилити (wdbc-egll); gift.khorne.livingWeapon — НЕ подключён, см. комментарий выше",
  ...
  "gift.khorne.livingWeapon": {
    label: "Полудействие+1 Бесчестия: до конца боя оружие/импровизированное оружие в руке нельзя выбить, +10 WS, Баланс до 0, Pen до Cor.b, теряет Primitive/получает Reinforced (импровизированное: без штрафа −20, +1 кубик, ×2 S.b). Разбирался под combat.cannotBeDisarmed (wdbc-egll) — НЕ подключён: тут иммунитет условный («до конца боя/сцены» после активации), а не постоянный, как у Tentacle/Extra Arm, системе негде хранить «активна ли способность сейчас» — сперва нужна тоггл-инфраструктура (wdbc-vyua), не одна строка capability.",
  ...
  "mutation.tentacle": {
    label: "+20 на тесты Борьбы (Сжать/Метнуть, module/combat/grapple.mjs) не подключено — своя точка входа, не приём; растяжение до 4м/узкие места — флейвор; 10 субмутаций не автоматизированы целиком (wdbc-vkwe), кроме решённых по одной отдельными тикетами (см. соседние коммиты по субмутациям 1/2-3/8/10). Субмутация 4-5 «С Присосками» (wdbc-egll) — теперь заведена: «нельзя выбить/вырвать» через новую переиспользуемую capability combat.cannotBeDisarmed (см. эту запись отдельно), «+30 Карабканье» через новый scopeTarget «climbing» (item-rules.mjs/resolve-test.mjs), подхватывается module/combat/movement-actions.mjs::showClimbDialog. Тот же новый scope «climbing» пригоден и для идентичных бонусов Wings/Крылья и Tail/Хвост — их субмутации им пока не заведены, отдельная правка на будущее.",
    source: "Мутация: Tentacle (Общие мутации)",
    reader: "module/sheets/attack-dialog.mjs — resolveSelection (+20 на приём Захват, maneuverKey===\"grapple\"); субмутация 4-5 — combat.cannotBeDisarmed + scopeTarget climbing (см. отдельные записи)"
  },
