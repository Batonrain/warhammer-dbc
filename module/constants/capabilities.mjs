...
export const CAPABILITIES = {
  ...
  "mutation.polymath": {
    label: "+10 тесты Ремесла (Trade) механизировано отдельной записью kind:\"testMod\" (modScope:skill, skillKey:trade — testMod ловит групповой Навык через ctx.group той же строкой, что и одиночный, см. resolve-test.mjs::effectAppliesTo) на этом же предмете. Capability покрывает ТОЛЬКО остаток: +10 на тесты Исследований (какой Навык книга не уточняет однозначно, не мигрировано) и Крит на любом из этих тестов — 1d5 Усталости + доп. тест немедленно (нет вида записи под этот триггер)",
