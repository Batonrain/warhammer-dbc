...
export const CAPABILITIES = {
  ...
  "mutation.tentacle": {
    label: "+20 на приём Захват, на 5 тестов Борьбы (Заломить/Пересилить/Вырваться/Выкрутиться/Перехватить Контроль) и на Укус (тоже настоящий тест WS/BS); Сжать/Хруст броска не делают вовсе (первое — штраф без теста, второе — автопопадание по книге); Метнуть/Замахнуться не реализовано роллом вообще (отдельный пробел); растяжение до 4м/узкие места — флейвор. Из 8 строк таблицы субмутации 2-3 (Natural Armour 3) и 8 (+10 S, Unnatural S+2) заведены Механикой самого предмета (packs-src); 1 (растяжение до 15м + подъём союзника) получила флаг-заглушку про запас (mutation.tentacle.longReach, wdbc-nc8q) — числового бонуса нет, действие целиком за ГМом; 9 «Изменчивое» (wdbc-2ynk) — форма руки/щупальца переключается кнопкой на листе предмета (не через capabilityKey — своя пара флаг+UI, module/apps/tentacle-hand-form.mjs), цена спишет module/combat/capability-cost.mjs, бонус +20 на приём Захват гасится, пока предмет в форме руки; 10 (Отделяемое, wdbc-1f5j) реализована напрямую в grapple.mjs (асимметричный выход из Захвата + информационный таймер регенерации 3ч через rules/cooldown.mjs) — без записи в этом реестре, читает item.system.submutation.label напрямую, не hasRuleFlag. Остальные 3 не автоматизированы: 4-5 бонус Карабканья — в системе нет отдельного скоупа теста «Карабканье» (это Athletics, тот же скоуп, что у Заломить/Пересилить — общий testMod задел бы их тоже); 6/7 — свойство оружия (Flexible/Corrosive+Toxic) именно на безоружных атаках щупальцем, для чего нет узкого крепежа",
    source: "Мутация: Tentacle (Общие мутации)",
    reader: "module/sheets/attack-dialog.mjs — resolveSelection (+20 на приём Захват, maneuverKey===\"grapple\", гасится tentacleBonusSuppressed при субмутации 9 в форме руки); module/combat/grapple.mjs — tentacleTechDef (+20 на 5 тестов Борьбы), _doBite (+20 на Укус), detachableTentacle/isDetachedGrapple (субмутация 10)"
  },
