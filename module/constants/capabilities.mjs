...
export const CAPABILITIES = {
  ...
  "mutation.tentacle": {
    label: "+20 на тесты Борьбы (Сжать/Метнуть, module/combat/grapple.mjs) не подключено — своя точка входа, не приём; растяжение до 4м/узкие места — флейвор; 10 субмутаций не автоматизированы (wdbc-vkwe), кроме решённых по одной отдельными тикетами (см. соседние коммиты по субмутациям 1/2-3/8/10). Субмутация 4-5 «С Присосками» (wdbc-egll) — намеренно оставлена флейвором целиком: (а) «нельзя выбить/вырвать удержанный предмет» — во всей системе нет кода, проверяющего разоружение/захват предмета из рук в бою (тот же непроверенный пробел у gift.khorne.livingWeapon и у идентичного флейвора «Опутанная Корнями»/дрюкхарийских биоимплантов — ни один из них тоже не заведён капабилити); (б) «+30 Карабканье» — в движке правил вообще нет отдельного scopeTarget «климбинг» (Карабканье — свой диалог module/combat/movement-actions.mjs::showClimbDialog, не читающий реестр testMod, тот же разрыв, что у Synesthesia с attack-dialog.mjs), тот же пробел у идентичных бонусов Wings/Крылья и Tail/Хвост (тоже не механизированы). Прецедент того же решения — субмутация 10 «Отделяемое» (wdbc-1f5j, тот же файл): бесповоротно отвергает bespoke-архитектуру ради одной строки d10.",
