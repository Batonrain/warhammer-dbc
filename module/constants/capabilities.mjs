...
export const CAPABILITIES = {
  ...
  "wraithbone.spiritTalk.possessConstruct": {
    ...
    reader: "module/combat/spirit-talk.mjs (wdbc-q30d) — triggerSpiritTalk (кнопка на вкладке БОЙ, 2 ОД, до 3 раз за сессию), applySpiritTalkPossession/processSpiritTalkRoundStart встраивают Combatant цели в очередь ходов сразу за кастером на F.b раундов (та же инфраструктура extra-turn.mjs/паттерн раундового хука, что Last Actor/Middle of the Hunt, wdbc-1rno). НЕ смоделировано: характеристики/таланты/навыки персонажа на конструкте (у Техники в схеме вообще нет characteristics — заводить временный оверлей ради одной находки не оправдано, эффект остаётся на столе); дальность W м и предел размера «не больше призрачного лорда» (не мерится движком, тот же принцип, что остальные Певцы Кости); сторона цели встречного теста WP+0 (Техника не несёт характеристику Воли) — автоматизирована только сторона персонажа (WP+0/Fel+10 на выбор системы, лучшее), исход подтверждает стол диалогом (тот же приём, что Deadly Effectiveness); гейт манифестации психосил vs исполнения песен через конструкт."
  },
