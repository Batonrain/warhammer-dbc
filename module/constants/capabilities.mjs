...
export const CAPABILITIES = {
  ...
  "mutation.tentacle": {
    label: "+20 на приём Захват, на 5 тестов Борьбы (Заломить/Пересилить/Вырваться/Выкрутиться/Перехватить Контроль) и на Укус (тоже настоящий тест WS/BS); Сжать/Хруст броска не делают вовсе (первое — штраф без теста, второе — автопопадание по книге); Метнуть/Замахнуться не реализовано роллом вообще (отдельный пробел); растяжение до 4м/узкие места — флейвор. Из 8 строк таблицы субмутации 2-3 (Natural Armour 3) и 8 (+10 S, Unnatural S+2) заведены Механикой самого предмета (packs-src); 1 (растяжение до 15м + подъём союзника) получила флаг-заглушку про запас (mutation.tentacle.longReach, wdbc-nc8q) — числового бонуса нет, действие целиком за ГМом. Остальные 5 не автоматизированы: 9/10 — ситуативные действия, не число; 4-5 бонус Карабканья — в системе нет отдельного скоупа теста «Карабканье» (это Athletics, тот же скоуп, что у Заломить/Пересилить — общий testMod задел бы их тоже); 6/7 — свойство оружия (Flexible/Corrosive+Toxic) именно на безоружных атаках щупальцем, для чего нет узкого крепежа",
    source: "Мутация: Tentacle (Общие мутации)",
    reader: "module/sheets/attack-dialog.mjs — resolveSelection (+20 на приём Захват); module/combat/grapple.mjs — tentacleTechDef (+20 на 5 тестов Борьбы), _doBite (+20 на Укус)"
  },
