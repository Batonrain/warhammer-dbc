...
// Коды из строки WC корпуса (напр. «Н1, ПБ1, ЛБ1») → ключ позиции. Экспорт —
// та же таблица переиспользуется для валидации Location Requirements узла
// (module/hooks.mjs, wdbc-qhwb): дуга узла бывает введена и кириллицей, и
// уже английским ключом (prow/dorsal/...), нормализация должна понимать оба.
