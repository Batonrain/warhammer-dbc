...
export const CONDITIONS = {
  bleeding: {
    ...
    desc: "В конце каждого Хода — бросок d10: на 1-5 персонаж получает 1 уровень Обескровливания, на 0 и меньше — умирает. Снимается полудействием, тестом Medicae−10 (−30, если пациент активно действовал в прошлый Ход или медик лечит себя); со жгутом — полное действие, +40.",
  ...
  helpless: {
    ...
    desc: "Не может совершать Физические действия. Рукопашная и выстрел в упор/в рукопашной по нему автоматически успешны и наносят удвоенный урон. Прочая стрельба получает +30. ОД и Реакции на Физические действия не тратятся (приказы голосом и психические ритуалы — можно).",
  ...
  // прямому запросу пользователя на основе находки wdbc-1rno — субмутация
  // «Оглушение» Fruit of Flesh/Плода Плоти втягивает это Состояние в плод).
  // Мех. эффект новый и намеренно узкий (только то, что даёт цитата книги):
  // TOTAL_IMMUNITY-гейт в combat/damage.mjs::applyDamageToActor. «Считается
  // Беспомощным» — та же чисто описательная связь, что уже у unconscious
  // выше (в реестре нет кросс-влияния Состояний друг на друга, только текст
  // тега). «Невозможно сдвинуть с места» — ЧЕСТНО не автоматизировано: в
  // системе нет ни одной механики принудительного перемещения (толчок/
  // нокбэк) ни у кого, гейтить нечего ни для одной находки, не только этой.
  stasis: {
    label: "Стазис", icon: "⏳", counter: "rounds", color: "#8fd0ff",
    desc: "Заморожен во времени: абсолютно неуязвим к любому урону (не только физическому — прямая цитата книги: «даже пинок Титана неспособен... как-то повредить»), считается Беспомощным. Книга также говорит «невозможно сдвинуть с места» — в системе нет ни одной механики принудительного перемещения, гейтить нечем.",
    body: `<circle cx="8" cy="8" r="6" fill="none" stroke="currentColor" stroke-width="1.3"/>
     <path fill="none" stroke="currentColor" stroke-width="1.3" stroke-linecap="round" d="M8 4.5V8L10.5 10"/>`
  },
  // Сладкий Туман (Wrapped in Chaos/Укутанный в Хаос, субмутация "6",
  // wdbc-1rno) — «эффект газа длится 3 часа ПОСЛЕ вдыхания», не привязан к
  // тому, стоит ли персонаж в облаке прямо сейчас. Единственное Состояние
  // с worldTime-длительностью (не Раунды/уровень/счётчик — тех трёх типов
  // для «часов» не хватает): доп. поле sweetMistExpiresAt (тот же приём,
  // что burningSourceDamage/burningGraceRounds выше — ручная надстройка
  // сверх стандартного счётчика, data/actor/_creature.mjs) хранит момент
  // истечения; хук истечения — module/apps/wrapped-in-chaos.mjs::
  // sweepSweetMistExpiry, зовётся из ТОГО ЖЕ Hooks.on("updateWorldTime", …)
  // в warhammer-dbc.mjs, что уже двигает виджет Календаря (по прямому
  // указанию пользователя — «привязать к виджету Календаря»), не отдельным
  // новым хуком. Штраф −10 на тесты W (до −20 vs Charm) — ОПИСАТЕЛЬНЫЙ, как
  // у большинства Состояний книги (poisoned/dazed и т.п.): в системе нет
  // центральной точки резолва тестов W (в отличие от Атаки, где Оглушение/
  // Повален уже читаются автоматически, module/sheets/attack-dialog.mjs) —
  // подключать который под одно Состояние значило бы трогать десяток разных
  // диалогов теста ради одной находки.
  sweetMist: {
    label: "Сладкий Туман", icon: "🌸", color: "#ff8ad1",
    desc: "Вдохнул газ соблазна (Wrapped in Chaos/Укутанный в Хаос) — −10 на все тесты W, до −20 на W vs Charm, 3 часа после вдыхания. Снимается само по истечении срока.",
    body: `<path fill="currentColor" d="M8 3C6.5 3 5.5 4.3 5.8 5.7 4.4 5.4 3 6.4 3 8s1.4 2.6 2.8 2.3C5.5 11.7 6.5 13 8 13s2.5-1.3 2.2-2.7C11.6 10.6 13 9.6 13 8s-1.4-2.6-2.8-2.3C10.5 4.3 9.5 3 8 3z"/>
     <circle cx="8" cy="8" r="1.6" fill="#06140d"/>`
  },
  // Мононить «Поцелуй Мимика» (Volunteer Actor/Доброволец Актёр, Аэльдари —
  // Атхистаур/Мимик, wdbc-ux8a) — «лишается речи и движения (в т.ч.
  // механодендритов/имплантов)», пока мононить не разорвана/не удалена.
  // Как и у helpless выше, «не может Физически действовать» — ОПИСАТЕЛЬНО
  // (нет общего isHelpless()-гейта в системе ни у одного Состояния, кроме
  // ручных проверок в attack-dialog.mjs) — не автогейтит Действия персонажа
  // кодом, только подпись/бейдж. Единственный МАШИНОЧИТАЕМЫЙ кусок — доп.
  // поле mimicWireBlocksPowers (тот же приём ручной надстройки сверх
  // Состояния, что у burningSourceDamage/sweetMistExpiresAt): «за доп. 10
  // сек/1м можно лишить цель психосил/техночудес» — читается на входе в
  // манифестацию психосилы (module/sheets/tabs/psychic.mjs), как уже делает
  // highSorceryManifestBlocked у Perfect Sorcerer. Кем цель управляется —
  // отдельный флаг flags.warhammer-dbc.controlledBy (module/rules/
  // actor-control.mjs), не это Состояние: управлять можно, не разрывая
  // нить, а нить/Состояние может стоять и без активного управления.
  mimicWire: {
    label: "Мононить (Поцелуй Мимика)", icon: "🕸️", color: "#c9a0ff",
    desc: "Обездвижен и лишён речи мононитью (в т.ч. механодендриты/импланты) — Физические действия по столу считаются недоступны. Может быть под контролем другого персонажа (module/rules/actor-control.mjs). Обрыв нити без хирургии — смерть; хирургическое извлечение — 16ч + тест Medicae(I)−40.",
    body: `<g fill="none" stroke="currentColor" stroke-width="1.1">
     <path d="M2.5 4L13.5 12M13.5 4L2.5 12M8 1.5V14.5M2 8H14"/></g>`
  },
  // Паразитический контакт (Трейт Parasite/Паразит, wdbc-ux8a — общий
  // механизм заражения, не только у Опарыша-Паразита): «сохранять контакт
  // как минимум 1d5 Ходов +1 Ход за каждый уровень брони в точке контакта».
  // counter:"rounds" — генерик-цикл processConditionTurnStart (combat/
  // condition-ticks.mjs) декрементирует САМ каждый Ход, спец-хук на 0
  // завершает заражение (тот же приём, что уже даёт возврат Очка Бесчестия
  // у Blessed Fits на том же 0). Партнёр (кто заражает) — доп. флаг
  // parasiticContactSourceUuid (тот же приём, что PARTNER_FLAG у Борьбы,
  // combat/grapple.mjs), не отдельное Состояние на источнике — с его стороны
  // ничего не меняется, пока заражение идёт.
  parasiticContact: {
    label: "Паразитический контакт", icon: "🪱", counter: "rounds", tickLabel: "Заражение", color: "#8a9a4e",
    desc: "Паразит держит контакт — до конца счётчика заражение завершится (полный контроль над телом). Можно сопротивляться: атаковать паразита или сорвать его (Борьба, приём «Вырваться») — при срыве 1d5 непоглощ. Rending в торс.",
    body: `<circle cx="8" cy="8" r="5.5" fill="none" stroke="currentColor" stroke-width="1.3"/>
     <path fill="none" stroke="currentColor" stroke-width="1.3" stroke-linecap="round" d="M8 8L11.5 5.5M8 8L11 9.5M8 8L5 10.5"/>`
  },
  blinded: {
    label: "Ослеплён", icon: "🙈", counter: "rounds", tickLabel: "Ослепление", color: "#cf9fff",
    desc: "Не видит: автопровал BS, −30 на WS и тесты, требующие зрения. Весь незнакомый ландшафт — Трудный, −20 против настоящего Трудного Ландшафта. Все атаки по нему — Незримые. Sonar Sense / Unnatural Senses снимают все штрафы.",
  ...
  burning: {
    ...
    desc: "В конце Хода 1d10 E(Fl) урона (у некоторых источников больше) мимо брони и 1 Усталости при непоглощённом уроне (иначе тест T+0). В начале Хода — тест W+0 или пропуск Хода в панике. Тушится полудействием (себе A−20, другому A+0).",
  ...
  // Врасплох (стр. 12, wdbc-x1nz.2.26): «пропускает свой первый Раунд боя и
  // не получает Реакции в этот Раунд» — ставится ГМ вручную (диалог
  // «Добавить состояние», как и прочие книжные Состояния), сама зачистка —
  // одноразовая, combat/action-economy.mjs::resetActionEconomy гасит её на
  // первом же сбросе экономики действий носителя. Бонус атаки +30 по такой
  // цели остаётся отдельной ручной галочкой «Цель Врасплох» в окне атаки
  // (sheets/attack/mods.mjs) — она относится к КАЖДОЙ атаке отдельно, а не к
  // самому носителю Состояния, и не переносится сюда.
  surprised: {
    label: "Врасплох", icon: "😲", color: "#ffb347",
    desc: "Застигнут врасплох в начале боя: 0 ОД и 0 Реакций на свой первый Ход, Реакций нет и до него (в течение всего первого Раунда). Атаки по нему в этот Раунд — отдельная галочка «Цель Врасплох» (+30) в окне атаки. Снимается автоматически сразу после первого Хода.",
    body: `<path fill="none" stroke="currentColor" stroke-width="1.6" stroke-linecap="round" d="M4.5 6.2Q8 3.4 11.5 6.2M8 8.4V10"/>
     <circle cx="8" cy="12.2" r="0.9" fill="currentColor" stroke="none"/>`
  },
  // Опьянение (стр. 12, wdbc-x1nz.2.32): книга приводит его примером
  // «расфокусированного разума» рядом с Ярость/Галлюцинациями, но не даёт
  // отдельной формулы длительности/тяжести (в отличие от ядов, вызывающих
  // «состояние, близкое к алкогольному опьянению», — те свои эффекты несут
  // сами). Наложение/снятие — на решение ГМа; единственный МАШИНОЧИТАЕМЫЙ
  // эффект — гейт Ментальных действий (см. isMentalActionBlocked,
  // rules/predicates.mjs), тот же уровень автоматизации, что у Ярости и
  // Галлюцинаций рядом.
  intoxicated: {
    label: "Опьянение", icon: "🍺", color: "#e0a458",
    desc: "Разум расфокусирован (алкоголь и подобное): манифестация психосил и Техно-чудес недоступна, пока не пройдёт. Длительность/снятие книга не формализует — решает ГМ.",
    body: `<path fill="currentColor" d="M5 2h6l-.6 3H5.6L5 2Z"/>
     <path fill="none" stroke="currentColor" stroke-width="1.4" d="M5.6 5h4.8l-.7 7.2a1.5 1.5 0 0 1-1.5 1.3H7.8a1.5 1.5 0 0 1-1.5-1.3L5.6 5Z"/>
     <path fill="currentColor" opacity="0.5" d="M6.1 6.5h3.8l-.3 3H6.4z"/>`
  },
  pinned: {
  ...
  shocked: {
    ...
    desc: "Провал теста Страха бросает по таблице Шока (стр. 53) — от −10 на все тесты, кроме T, до полной неспособности действовать (степень зависит от результата броска). Штраф выпавшей строки и запрет действовать учитываются сами. Оправиться — тест WP+0 в начале каждого своего Хода (тест Морали, провал не снимает эффекты Командования); в первый Ход Шока у некоторых строк нельзя, у панического бегства — только вдали от источника.",
  ...
  // BONE-Head / Костеголов (Огрин): поле Haywire мощностью 3+ сбивает мозговой
  // имплант. Ставит combat/damage.mjs при попадании ЭМИ, со сроком — сколько
  // Ходов поле держится на 3+ (затухает на 2 за Ход); тесты I проваливает
  // правило rules/library/ogryn.mjs (ogryn.boneHead.haywire).
  implantHaywire: {
    label: "Сбой импланта", icon: "📡", color: "#8fd0ff",
    desc: "Мозговой имплант Огрина сбит полем Haywire (3+): не может читать, писать и считать больше, чем до пяти, автоматически проваливает все тесты I, ментальные действия занимают вдвое больше времени (свободное → полудействие). Держится, пока поле не ослабнет ниже 3 или Огрин не покинет его.",
    body: `<g fill="none" stroke="currentColor" stroke-width="1.5" stroke-linecap="round"><path d="M4 12.5C4 9 5.8 7.5 8 7.5S12 9 12 12.5"/><path d="M8 7.5V3M6.2 4.6L9.8 2.4M9.8 4.6L6.2 2.4"/></g>`
  },
  suffocating: {
    label: "Удушье", icon: "🫁", counter: "rounds", color: "#8fb0c4",
    desc: "Задержка дыхания: T.b минут в покое или T.b×2 Раундов в активных действиях; всё это время — тест T+0 каждую минуту/Ход, провал +1 Усталости. Не вздохнул за это время — без сознания, ещё через T.b Раундов — смерть. Режим (покой/активность) — кнопка на вкладке БОЙ.",
