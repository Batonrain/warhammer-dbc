...
//  rules       — машинная часть Особенности для реестра правил. Лежит в
//                rules/library/homeworlds.mjs, приходит актору источником
//                «homeworld» (rules/sources.mjs). Ею записаны Особенности,
//                которые система применяет САМА, а не показывает галочкой.
//
...
import { TEMPLE_RULES, DEATH_RULES, INDUSTRIAL_RULES, CEMETERY_RULES }
  from "../rules/library/homeworlds.mjs";
...
export const HOMEWORLDS = [
  ...
  },
  {
    ...
    // Бросок и возврат Очка — rules/fate-save.mjs, ловится на updateActor.
    rules: TEMPLE_RULES
  },
  {
  ...
  },
  {
    ...
    // Прежде здесь была галочка-напоминалка в диалоге атаки. Теперь окно атаки
    // само гасит «Цель Врасплох», если правило есть у ЦЕЛИ (sheets/attack-dialog.mjs).
    rules: DEATH_RULES
  },
  {
  ...
  },
  {
    ...
    // Прежде здесь была галочка ДО броска, хотя решение принимается ПОСЛЕ него.
    // Теперь кнопка появляется в карточке проваленного теста (combat/fear.mjs).
    rules: CEMETERY_RULES
  },
  {
  ...
  },
  {
    ...
    // Лимит помощников считает rules/assists.mjs — такой помощник не занимает слот.
