...
export function creatureSchema({ granted = false } = {}) {
  ...
  return {
    ...
    // типов, а у существ не было ни у одного — и открыв демона из Бестиария,
    // ГМ не мог узнать, из какой он книги. Лежит в общей части, а не в схеме
    // Демона: Персонажи Бестиария (83 из 115) книжные ровно так же.
    ...
    // Ярость (Frenzy) — ручной тумблер состояния боя, тот же приём, что
    // meleeStance: полная механика Ярости (+10 WS/S/W, −20 BS/I/F, стр. 15)
    // НЕ реализована (только capability-заглушки berserker.core.frenzy и
    // т.п.) — это поле нужно только как ГЕЙТ для условных возможностей вроде
    // Дара Кхорна «Purity of Wrath» (weaponPropertyImmunity.* только в
    // Ярости, wdbc-plsf), см. rules/mech-when.mjs requiresRage.
    inRage:        bool(false, "В Ярости"),
    ...
    // Свойство оружия Corrosive (X), wdbc-plsf: стойкая потеря AP в месте
    // попадания (после расчёта урона), пока броню не починят (Trade(Armourer)
    // +0, ½ смены). Вычитается из armorAP в prepareDerivedData (documents/
    ...
    // частью тела — GM-отыгрыш (в системе нет понятия «тест использует эту
    ...
    // (снимается любой мед. помощью/полным исцелением). Триггер «оба ОД на
    // физическое действие» не отслеживается автоматически (нет разметки
    // «физическое» на действиях в economy) — рана хранится списком, урон
    // наносится кнопкой в чате по решению игрока/ГМ.
    crippledWounds: objList("Раны с шипами (Crippling)"),
