...
export class WeaponData extends foundry.abstract.TypeDataModel {
  ...
  static defineSchema() {
    ...
    return {
      ...
      // Заклинило (wdbc-vwfk) — впервые становится РЕАЛЬНЫМ состоянием, а не
      ...
      // drukhari чуть выше — данные проставляются при авторинге предмета
      // (см. packs-src), готовой UI-галочки на листе нет ни у одного из этих
      // двух флагов; отметить новый/самодельный предмет можно правкой поля
      // напрямую. Засеяно true только на предметах, где психокостяная
      // природа однозначна текстом/папкой пака (Психокостяное, Азуриане) —
      // неоднозначные случаи (напр. броня с частично психокостяными
      // элементами) сознательно не тронуты, см. notes Reformation Song.
      wraithbone:       new BooleanField({ initial: false, label: "Психокостяное" }),
      // Иммунно к Reformation Song (сложный принцип работы или мистическая/
      // божественная природа, книжная оговорка) — читается ТОЛЬКО вместе с
      // wraithbone:true. По умолчанию false везде: у книги нет отдельного
      // списка «что именно иммунно», это описано как ситуативное решение
      // ГМа за столом — поле здесь ради того, чтобы решение ГМа один раз
