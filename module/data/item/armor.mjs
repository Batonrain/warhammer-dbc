...
export class ArmorData extends foundry.abstract.TypeDataModel {
  ...
  static defineSchema() {
    ...
    return {
      ...
      // Состояние «пробита» между ударами (wdbc-k0ff) — общий трекер, не
      // завязанный на конкретное свойство: ставится, когда попадание в
      // локацию, которую покрывает этот предмет, дало непоглощённый урон
      // (module/combat/damage.mjs::applyDamageToActor). Снимается только
      // вручную (ремонта как действия в системе нет) — читают свойства вроде
      // Sealed («теряется при пробитии»), сам смысл потери решает читатель,
      // не этот флаг.
      breached:     new BooleanField({ initial: false, label: "Пробита" }),
