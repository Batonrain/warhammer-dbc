...
export const ASTARTES_RULES = [
  ...
  },
  {
    ...
    // лечение, при котором Астартес всегда считается отдыхающим. Усиленный
    // профиль безоружного удара (стр. 40) сюда больше не зашит — это теперь
    // Талант Unarmed Warrior/Безоружный Воин (packs-src/talents/Пугилист),
    // который Астартес получает как один из стартовых (см. Mechanics расы,
    // packs-src/races/Люди/Astartes...) — доступен и любому другому, кто его
    // купит, а не только Астартес хардкодом.
