...
// ОБЩИЙ примитив (по прямому запросу пользователя): system.incomingDamageReduction
// (rules/character.mjs, combat/damage.mjs) раньше заполнялся ТОЛЬКО
// наркотиками жёстко зашитым циклом — эта функция ВТОРОЙ, независимый
// источник того же поля, живой cross-actor источник (тот же приём, что уже
// даёт rules/psychic-sustain-target.mjs — считается заново на каждый вызов,
// ничего не хранится на цели, только на владельце — исключения). Знак:
// отрицательное значение = МЕНЬШЕ поглощения = БОЛЬШЕ входящего урона (то
// же самое поле, тот же знак, что и наркотики выше).
