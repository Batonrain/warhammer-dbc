 ...
 *                    записаны с нужным знаком для этой стороны. По той же
 *                    логике «−150°..+150°» (Василиск/Медуза/Химера и т.п.,
 *                    спарка на люке) даёт ШИРОКУЮ дугу 300°/центр 0° — почти
 *                    круговой обзор с узкой мёртвой зоной 60° точно сзади,
 *                    а не наоборот.
 ...
 *
 * wdbc-n8m: до этой правки a/b подставлялись в формулу «как записаны в
 * строке», без приведения к каноничному диапазону (−180,180] — пока данные
 * пака всегда писали оба конца уже в этом диапазоне (конвенция «записаны с
 * нужным знаком для этой стороны» из абзаца выше), результат совпадал с
 * модульной арифметикой случайно. Но конец, ЗАПИСАННЫЙ через переход
 * 0°/360° буквально (например «350°..+30°» вместо канонического «−10°..+30°»
 * для того же самого сектора у передней дуги) при чтении «как есть» без
 * нормализации давал совершенно другую (и неверную) дугу: центр 190°/ширину
 * 320° вместо верных центра 10°/ширины 40°. Теперь каждый конец сначала
 * приводится к (−180,180] через normalizeAngle180 (по кругу, эквивалент
 * модульной арифметики) — для уже каноничных чисел (все текущие 27 значений
 * пака) это тождественное преобразование и ничего не меняет, а для записи
 * через 0°/360° даёт правильный результат. Само вычитание/усреднение a и b
 * после нормализации не меняется — это по-прежнему «читать как есть», не
 * «кратчайший путь между a и b» (см. канонический пример «−150°..+150°»
 * выше: кратчайший путь дал бы неверные узкие 60° вместо верных широких 300°).
 * @param {string} spec
