...
// unseen-параметр attack.mjs (structural wp.unseen ИЛИ Сокрытая Угроза ИЛИ,
// с wdbc-1rno.3, геометрия «Скрытной Атаки» — attack.mjs::sneakAttackUnseen,
// combat/facing.mjs::isOutsideDefenderView). ОБНОВЛЕНО (wdbc-1rno.3): «со
// спины, вне поля зрения цели» больше не только САМЫЙ ЧАСТЫЙ способ по
// формулировке — теперь и реально проверяемый движком источник Незримого,
// а не только structural/Сокрытая Угроза. Остающаяся честная граница —
// снимок геометрии на МОМЕНТ атаки, не слежение за позицией весь Ход (см.
// facing.mjs) — RAW «весь свой Ход вне поля зрения» шире того, что здесь
// проверяется, но это уже не «нет учёта facing вовсе», а конкретное и
// более узкое приближение.
