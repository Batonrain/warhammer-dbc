...
/**
 * Истёк ли срок. Решаем по остатку, а не по флагу expired: остаток ядро
 * пересчитывает на каждой подготовке данных, а флаг ведёт отдельный механизм
 * слежения, и он бывает поднят тогда, когда срок на самом деле не вышел.
 *
 * НЕСЧИТАЕМЫЙ остаток (Infinity) — это «посчитать нечем», а не «вышел»
 * (wdbc-tr02). Срок в РАУНДАХ вне боя ядро посчитать не может и ставит
 * Infinity, при этом на любой тик мирового времени само поднимает expired:
 * вне боя оно считает удовлетворённым любое боевое событие истечения
 * (client/documents/active-effect.mjs::isExpiryEvent, «Outside of combat,
 * treat any time advancement as satisfying in-combat expiry events»).
 * Поверить флагу в этот момент значит снять «Оглушение на 2 раунда» ещё до
 * первого удара — ровно это живая проверка и увидела.
 *
 * Флаг остаётся единственным источником в одном случае: у сырых данных, по
 * которым ядро вообще не проходило и поля remaining нет.
 *
...
export function isDurationExpired(duration) {
  ...
  if (duration?.remaining !== undefined) return false;   // посчитать нечем — значит не истёк
...
/** Единицы, привязанные ко ВРЕМЕНИ мира (в отличие от раундов и Ходов боя). */
const TIME_UNITS = new Set(["years", "months", "days", "hours", "minutes", "seconds"]);

/**
 * Меряется ли этот срок мировым временем. Раунды и Ходы — дело боевого
 * трекера, и подметать их по ходу мирового времени нельзя: вне боя их
 * остаток не считается вовсе (см. isDurationExpired выше).
 */
export function isTimeBasedDuration(duration) {
  return TIME_UNITS.has(String(duration?.units || ""));
}
