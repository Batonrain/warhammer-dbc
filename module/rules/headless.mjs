...
//
// Место попадания: «Голова» и «Глаз (Голова)» → «Торс», одной точкой в
// combat/damage.mjs::applyDamageToActor, как Бронзовый Мирмидон
// (rules/bronze-myrmidon.mjs). «Сочленение / Шея» НЕ переводится: это общая
// метка суставов конечностей и шеи, какая из них — не различить; решение за
// владельцем (вопрос в wdbc-1rno.20). Карточка атаки строится раньше, чем
...
export function redirectHitLocationForHeadless(hitLocation, actor) {
  ...
  if (hitLocation === "Голова" || hitLocation === "Глаз (Голова)") return "Торс";
  return hitLocation;
}
