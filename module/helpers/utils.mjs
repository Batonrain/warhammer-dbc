...
// Подъём 900 вместо 1350 и Толкание 1350 вместо 1800.
//
...
//
// Толкание тоже раньше не бралось из книги, а считалось формулой ×2 от
// Подъёма (=×4 от Ношения). На суммах 0-3 и 23+ книга сама следует этой
// формуле почти везде, поэтому расхождение было незаметно — но на суммах
// 4-22 книга даёт заметно меньшее Толкание (напр. при сумме 4: 36, не 72;
// у Астартес на сумме 16: 1800, не 2700), а на 26 у неё собственная
// опечатка (25 000 вместо формульных 25 200). wdbc-x1nz.2 — сверка Гл.II.
//
...
// строка 4 — Толкание 36 равное Подъёму, строка 26 — Толкание 25 000, а не
// 25 200 по формуле ×2 от Подъёма. Строка 7 — Толкание 114, а не 144 по
// соседнему шагу: сверено по таблице core.json при приёмке стопки
// #482-#504, это книга, а не опечатка транскрипции.
const CARRY_TABLE = [
  [0.9, 2.25, 4.5],       [2.25, 4.5, 9],          [4.5, 9, 18],            [9, 18, 36],
  [18, 36, 36],           [27, 54, 72],            [36, 72, 108],           [45, 90, 114],
  [56, 112, 180],         [68, 134, 224],          [78, 156, 268],          [90, 180, 312],
  [112, 224, 360],        [125, 450, 448],         [337, 674, 900],         [450, 900, 1348],
  [675, 1350, 1800],      [900, 1800, 2700],       [1350, 2700, 3600],      [1800, 3600, 5400],
  [2250, 4500, 7200],     [2900, 5800, 9000],      [3550, 7100, 11600],     [4200, 8400, 16800],
  [4850, 9700, 19400],    [5500, 11000, 22000],    [6300, 12600, 25000],    [7250, 14500, 29000],
