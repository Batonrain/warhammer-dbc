 ...
 * «умирает»/«убивает»/«погибает» (все три книжных глагола смерти),
 * «безжизненн-», «труп» (без «-ный»), «уходит жизнь», «не способна/не
 * удаётся пережить», «смерть наступает», «заканчивается мгновенной
 * смертью», «это смертельно».
 */
export function textAssertsDeath(text) {
  if (!text) return false;
  return /умира[а-яёА-ЯЁ]*|убива[ею][а-яёА-ЯЁ]*|погиба[ею][а-яёА-ЯЁ]*|безжизненн[а-яёА-ЯЁ]*|труп(?!н)[а-яёА-ЯЁ]*|уходит\s+жизнь|не\s+(?:удаётся|способна)\s+пережить|смерть\s+наступает|заканчивается\s+мгновенной\s+смертью|это\s+смертельно/giu
    .test(text);
}
