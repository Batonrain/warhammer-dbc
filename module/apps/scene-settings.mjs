...
export class SceneSettingsApp extends HandlebarsApplicationMixin(ApplicationV2) {
  ...
  _onRender(context, options) {
    ...
    // _onRender. Состояние (env/veil) не общее — но el общий, а разметка
    // обоих партиалов вставляется в DOM ОДНОВРЕМЕННО (вкладки различаются
    // только классом active, скрытая вкладка остаётся в разметке) — так что
    // селекторы вроде [data-act=reset] у обеих кнопок совпадают буквально,
    // и без уточнения найдётся первый элемент в документе (Окружение, оно
    // в разметке раньше). Это стоило реальной находки (wdbc-gyj): кнопка
    // «Сброс» Окружения стирала Завесу целиком, а кнопка «Сброс» Завесы не
    // срабатывала вовсе. Каждый _onRender обязан сузить такие селекторы до
    // границ своего контейнера (.wh-env / .wh-veil-app) сам — единственного
    // источника разметки на двоих для этого недостаточно.
    this.env.element = el;
