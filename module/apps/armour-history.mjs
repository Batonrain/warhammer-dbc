...
//
// Откат при снятии/перебросе: cohesion откатывается по флагу cohesionApplied
// (reconcileCohesionForActor умеет откатывать и без записи на предмете);
// corruption применённое остаётся — Порча по книге не «снимается» вместе
// с особенностью, это разовое событие (wdbc-sg57).
