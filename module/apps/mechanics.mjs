...
/**
 * Готова ли запись к запуску — троттлинг (учитывает scriptThrottleMax > 1 —
 * счётчик, не единичный gate) И, если задана, доступность пула цены
 * (wdbc-suwp: без актора-владельца платная запись недоступна — списывать
 * цену не с чего). Экспортирована (wdbc-1rno): тот же gate нужен и кнопке
 * «▶ Запустить», и автозапуску по исходу теста (rules/kind-outcome.mjs) —
 * один на оба пути, не дублируется.
 */
  ...
  switch (entry.kind) {
    ...
    case "script": {
      ...
      const costLabel = entry.capabilityCostPool
        ? ` — цена: ${capabilityCostLabel({ pool: entry.capabilityCostPool, amount: entry.capabilityCostAmount })}`
        : "";
      if (!entry.scriptTrigger) return base + costLabel;
      const scope = REROLL_SCOPE_LABEL({ ...entry, rerollScope: entry.modScope });
      const triggerLabel = entry.scriptTrigger === "critFailure" ? "Критический Провал" : "Критический Успех";
      return `${base} — авто на ${triggerLabel}${scope ? ` (${scope})` : " (область не выбрана)"}${costLabel}`;
    }
    case "group": {
      const g = entry.group;
...
function buildEntryWhenHtml(groupId, ent, canEdit, item = null) {
  ...
  // «Когда Покровитель» (wdbc-xxb7) — седьмой независимый гейт: список ключей
  ...
  </div>${subHtml}${talentHtml}${tierHtml}${rageHtml}${sealedArmourHtml}${patronHtml}`;
}
