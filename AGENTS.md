...
**Живые проверки в Foundry сессия сама не делает — даже через агента**
(правило с 03.09.2026, отменяет и версию 02.09.2026 про синхронный вызов
`live-tester`, и накопитель `wdbc-5009` одним тикетом). Реализовал что-то, что
по-хорошему нужно открыть и прощёлкать в мире — не лезть в живой мир самому,
не звать `live-tester` синхронно, а завести отдельный bd с меткой `testing`
(`bd create --type=task --label=testing`, при наличии PR — `--external-ref
gh-<N>`) и продолжить свою работу, не дожидаясь Foundry. Актуальный порядок —
`bd recall cmd-testirovanie`: отдельная сессия по команде «Тестирование»
(скилл `testirovanie`) собирает все открытые `testing`-задачи, проводит их
через `live-tester`, найденные баги заводит отдельными bd (без метки
`testing`), проверенную `testing`-задачу закрывает сама — независимо от того,
нашёлся баг внутри или нет. Задачи с меткой `testing` никогда не попадают в
вывод «Список задач» — это очередь другой команды, не тематическая работа.
...
шагом тематическими ветками от `origin/main`),
