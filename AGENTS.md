...
| --- | --- | --- |
| Схема типа документа | `module/data/`, регистрация в `module/data/index.mjs` | не описывать тип, у которого есть схема, ещё и в `template.json` — запись там опустошается тем же коммитом |
...
Тип, зарегистрированный в `module/data/index.mjs`, `template.json` больше не
читает совсем. Приём на каждый новый тип:

1. класс `TypeDataModel` в `module/data/item/` (или `actor/`);
2. регистрация в `index.mjs`;
3. запись типа в `template.json` опустошается **тем же коммитом**;
4. две записи в таблице `test/data/item-schemas.test.mjs` — умолчания прежнего
   `template.json` и сохранность документов пака.
