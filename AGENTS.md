...
**Шаблон зоны поражения — НЕ реализован.** Ни конус/круг/линия (штатный
Foundry `MeasuredTemplate` API — нигде в системе не тронут), ни поиск
токенов внутри фигуры. Есть только смещение точки промаха Взрывного
(`module/combat/scatter.mjs`) — кто ещё попал под зону, код не считает;
`module/combat/attack-card.mjs` прямым текстом просит ГМа проверить
вручную. Полный разбор с file:line — «Атлас Механики» (ссылка в описании
тикета wdbc-1pa, раздел V).
