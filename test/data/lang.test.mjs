...
describe("словарь типов документов", () => {
  it("каждый тип актора из template.json подписан", () => {
    const missing = TEMPLATE.Actor.types.filter(t => !RU.TYPES?.Actor?.[t]);
    expect(missing).toEqual([]);
  });

  it("каждый тип предмета из template.json подписан", () => {
    const missing = TEMPLATE.Item.types.filter(t => !RU.TYPES?.Item?.[t]);
    expect(missing).toEqual([]);
  });

  it("лишних подписей нет — тип, которого больше нет, вычищается вместе со схемой", () => {
    expect(Object.keys(RU.TYPES.Actor).filter(t => !TEMPLATE.Actor.types.includes(t))).toEqual([]);
    expect(Object.keys(RU.TYPES.Item).filter(t => !TEMPLATE.Item.types.includes(t))).toEqual([]);
  });
});
