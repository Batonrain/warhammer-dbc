...
const DEVIATIONS = {
  ...
  squad: {
    // Получатель «Личной Команды» — структурная ссылка, заведена гораздо
    // позже template.json (wdbc-sk8s, Voice of God/Глас Божий: «получатель
    // Личной Команды тоже получает Очко Бесчестия» — раньше получателя
    // нигде не хранили, выбор был устным).
