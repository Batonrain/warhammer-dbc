...
// wdbc-oxdn: Борьба «Метнуть» (module/combat/grapple.mjs::_doThrow) — тот же
// приём без оружия, что и Пинок выше, но дальнобойный (BS+0, без Стойки/Базы/
// Локуса Сокрушения — рукопашных понятий, к дальнобойному тесту не относятся).
describe("приём без оружия — дальнобойный режим (techDef.ranged)", () => {
  const throwTech = { label: "Метнуть", ranged: true, bsBonus: 0,
                       damage: "1d5+S.b", damageType: "impact", pen: 0 };

  it("порог считается от BS, Стойка/База/Локус Сокрушения не подмешиваются", async () => {
    captured.dice = [30, 4];
    const actor = attacker({ meleeStance: "aggressive", meleeBase: "fullatk", fatigue: { value: 0 } });
    await showAttackDialogNoWeapon(actor, throwTech);

    // BS 45 + 0 (приём) = 45, без стойки/базы — они не про дальнобойное; бросок 30 → попадание.
    const card = captured.chat.at(-1).content;
    expect(card).toContain("BS: <b>45</b>");
    expect(card).toContain("Порог: <b>45</b>");
    expect(card).not.toContain("база");
    expect(card).toContain("Попадание");
  });

  it("усталость считается по BS (не освобождена, как у Т/Инф/Cog/ПС), но не удваивается стойкой/базой", async () => {
    captured.dice = [96];
    const actor = attacker({ fatigue: { value: 1 } });
    await showAttackDialogNoWeapon(actor, throwTech);
    // BS 45 - 10 усталость = 35.
    const card = captured.chat.at(-1).content;
    expect(card).toContain("Порог: <b>35</b>");
  });
});

// wdbc-oxdn: цель урона Борьбы (Метнуть/Замахнуться) — сам захваченный
// партнёр, а не то, что игрок навёл прицелом Foundry (в отличие от обычной
// атаки) — module/combat/grapple.mjs::_doThrow передаёт techDef.forceTargetActor.
describe("приём без оружия — принудительная цель урона (techDef.forceTargetActor)", () => {
  const partner = { uuid: "Actor.partner1", name: "Партнёр", system: { conditions: {} } };
  const swingTech = { label: "Замахнуться", wsBonus: 0, damage: "1d5+S.b",
                       damageType: "impact", pen: 0, forceTargetActor: partner };

  it("кнопка урона несёт UUID партнёра и его имя, не зависит от game.user.targets", async () => {
    setTargets([]);
    captured.dice = [10, 3];
    await showAttackDialogNoWeapon(attacker(), swingTech);

    // 1d5+S.b при S 40 (бонус 4): 3 + 4 = 7.
    const card = captured.chat.at(-1).content;
    expect(card).toContain('data-force-target="Actor.partner1"');
    expect(card).toContain("Применить урон: 7 → Партнёр (Торс)");
  });

  it("без forceTargetActor кнопка ведёт себя как раньше — без data-force-target, подпись «Торс»", async () => {
    captured.dice = [10, 3];
    await showAttackDialogNoWeapon(attacker(), { ...swingTech, forceTargetActor: undefined });

    const card = captured.chat.at(-1).content;
    expect(card).not.toContain("data-force-target");
    expect(card).toContain("Применить урон: 7 → Торс");
  });

  it("Беспомощность/авто-успех считаются у форсированной цели, не у game.user.targets", async () => {
    setTargets([]); // никто не наведён прицелом — обычный порог был бы промахом
    captured.dice = [96, 3]; // второй кубик — урон, авто-успех всё равно его кидает
    const helplessPartner = { uuid: "Actor.partner1", name: "Партнёр", system: { conditions: { helpless: true } } };
    await showAttackDialogNoWeapon(attacker(), { ...swingTech, forceTargetActor: helplessPartner });

    const card = captured.chat.at(-1).content;
    expect(card).toContain("Попадание");
    expect(card).toContain("Беспомощна");
  });
});
