...
describe("метки видны не только у Персонажа", () => {
  ...
  // Отбор — по Состоянию существа («Повален»), а не по наличию блока: у Орды блок
  // свой, из одного Подавления («Контроль Орды»), меток существа ей не считают.
  const typesWithConditions = Object.entries(ACTOR_DATA_MODELS)
    .filter(([, M]) => { try { return "prone" in (new M({}).toObject().conditions ?? {}); } catch { return false; } })
    .map(([type]) => type);
