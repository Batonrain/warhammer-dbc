...
function packFiles(pack) {
  ...
  // Нет каталога — ошибка, а не «ноль документов» (wdbc-bjy1.8): опечатка в
  // имени пака иначе делает тест вакуумно зелёным. Тип, чей пак выведен из
  // системы целиком (small-craft, 16.09.2026), объявляет это явно —
  // pack: null в test/data/item-schemas.test.mjs, сюда такой не приходит.
  const dir = path.join(PACKS_SRC, pack);
  if (!fs.existsSync(dir)) throw new Error(`packs-src/${pack}: каталога пака нет — опечатка в имени или пак выведен (тогда pack: null)`);
  walk(dir);
