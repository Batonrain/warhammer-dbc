...
// остатку пула и списывает при нажатии. Механизм был, а несколько Даров,
// которые по книге целиком сводятся к «потратить Очко и сделать X», цены не
// несли — игрок тратил очко по памяти и забывал.
//
...
//
// 08.09.2026 (wdbc-1rno): Eye of Challenge/Око Вызова выбыл из этого списка —
// «концентрация на цели» перестала быть «плати и отыгрывай», теперь несёт
// РЕАЛЬНУЮ запись kind:"script" (читает WS/S/Parry/Берсерк-Таланты настоящего
// актора цели, запускает срок с настоящим штрафом), и цена переехала на эту
// запись (см. capability-запись предмета — своей цены больше не несёт, чтобы
// не списывать дважды). Тот же путь миграции, что у Iconoclast/Father of
