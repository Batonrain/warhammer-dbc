...
describe("храповик: capability-заглушки в Мутациях/Дарах не растут молча (wdbc-1rno)", () => {
  ...
  it("Мутации/Дары: не больше 130 записей несут ТОЛЬКО пустую capability-заглушку", () => {
    ...
    // Infernal Armiger/Инфернальный Оруженосец у ВСЕХ 4 Богов разом (Шаг A:
    // панель МИНЬОНЫ показывает слугу без Таланта-слота; Шаг B: item.system.
    // noTest на Ритуале — castNoTestRitual катает без броска,
    // spawnDemonOnScene({asMinion}) сразу ставит system.masterUuid; Шаг C:
    // каждому Богу выдан готовый Ритуал «Призыв Связанного Оруженосца»,
    // призывающий ИМЕННО его демона из Бестиария и привязывающий его
    // Миньоном без слота) и Blood-Anointed/Кровопомазанник (Дар Кхорна) —
