...
describe("храповик: capability-заглушки в Мутациях/Дарах не растут молча (wdbc-1rno)", () => {
  ...
  it("Мутации/Дары: не больше 130 записей несут ТОЛЬКО пустую capability-заглушку", () => {
    ...
    // 12.09.2026 (изолированный PR wdbc-1rno-mutations-cluster, cherry-pick
    // одного сессионного коммита на чистый origin/main, без остального
    // локального бэклога — см. reference_pr-branch-without-checkout):
    // фактический счёт на origin/main на момент ветвления — 33 (не 35,
    // committed-порог там устарел относительно собственного дерева origin,
    // это отдельный долг origin/main, не трогается этой веткой). Эта ветка
    // добавляет ТОЛЬКО работу своей сессии (Дары Богов кластера Тзинч +
    // начало Общих Мутаций), реально измеренную инструментом храповика
    // (не оценкой на глаз): Devourer of Time/Пожиратель Времени (доп. ход
    // жертвам Врасплох, kind:"script"), Perfect Sorcerer/Совершенный
    // Чародей (гейт Высшего Колдовства без покровительства Хаоса
    // Неделимого), Personal Adaptation/Персональная Адаптация (растущий
    // бонус во Встречном тесте против конкретной цели), Sundering/
    // Разделение (два боевых клона персонажа при Провале Смерти),
    // Armour of the Gods/Доспехи Богов (автовыдача Архетипа Броненосец +
    // Божественная Броня через реальный конвейер applyItemMechanics),
    // Blessed Fits/Благословенные Припадки (Оглушение при провале
    // переброса за Бесчестие, возврат Бесчестия по истечении), Breeze/
    // Бриз (снятие потолка терминальной скорости при падении + иммунитет
    // к Удушью), Burned Senses/Выжженные Чувства (второй бросок «усиленного»
    // чувства, перманентное состояние при sight/hearing). Прочие находки
    // сессии (False Witness/Hidden Threat/Omniglot/The Unnameable) остались
    // честными заглушками — см. bd comment wdbc-1rno по каждой. 33 → 15.
    expect(stubOnly.length).toBeLessThanOrEqual(15);
  });
});
