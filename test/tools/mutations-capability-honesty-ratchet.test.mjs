...
describe("храповик: capability-заглушки в Мутациях/Дарах не растут молча (wdbc-1rno)", () => {
  ...
  it("Мутации/Дары: не больше 130 записей несут ТОЛЬКО пустую capability-заглушку", () => {
    ...
    // честными пробелами до следующих шагов серии (D/E/F). 49 → 45.
    // 09.09.2026 (продолжение, следующий архитектурный пробел): Blood-
    // Anointed/Кровопомазанник (Дар Кхорна) — щит-дефлектор 1-44 от
    // стрелковых/взрывов, если не стрелял и связан в рукопашной, тем же
    // тактом конца Хода, что уже даёт Щит Праздности (combat/turn-state-
    // shield.mjs::bloodAnointedRating, геометрия контакта — combat/free-
    // attack.mjs::enemyContactTokenDocs). «Шёл к врагу» (альтернативный
    // триггер) и эскалация «в крови» 44→88 остаются честными пробелами —
    // движок не хранит позицию начала Хода и не отслеживает состояние
    // «измазан кровью» вовсе. Фактический счёт (включая параллельно
    // смигрированные Дары Рыцаря Бога) — 41, порог подтянут до него.
    expect(stubOnly.length).toBeLessThanOrEqual(41);
  });
});
