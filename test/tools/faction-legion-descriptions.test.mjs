...
/**
 * Записи legions.mjs, у которых карточки Фракции нет, — каждая осознанно.
 *
 * Это не «руки не дошли до текста», а незаведённые узлы дерева
 * принадлежностей: Мастер создания эти ордена предлагает, а во Фракциях их
 * нет. Список заморожен, чтобы новый орден в константе без карточки ронял
 * тест, а не тонул молча.
 */
const NO_CARD = [
  "захватчики", "красные когти", "копья императора",
  "серебряные черепа", "освященные", "кархародоны",
];
...
describe("Фракции легионов и орденов несут правила из legions.mjs", () => {
  ...
  it("карточки нашлись — иначе гейт молчал бы впустую", () => {
    expect(list.length).toBeGreaterThanOrEqual(83);
  });
