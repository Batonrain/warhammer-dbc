...
//
// Фаза 7 конвейера: сборка карточки чата. Модуль ничего не считает, не бросает
// кубы и не читает документы Foundry — на вход приходят уже посчитанные числа,
// на выход идёт HTML.
//
...
//
// Заглушка Foundry здесь по-прежнему НЕ нужна, и это свойство файла: карточка
// собирается из готовых чисел, документов не читает. Переезд на общий сборщик
// (wdbc-ec0r) её было потребовал — esc в helpers/utils.mjs звал
// foundry.utils.escapeHTML без запасного пути, — но чинить это импортом
// заглушки в каждый такой тест значило бы расплатиться правилом проекта за
// одну строчку. Запасной путь добавлен в сам esc.
...
describe("карточка атаки", () => {
  ...
  // (и там дважды), из-за чего в живой игре Дар не срабатывал вовсе.
...
// ════════════════════════════════════════════════════════════════════════════
//  Распыление даёт цели ОДНУ защиту (wdbc-09t)
//
//  Книга (стр. 168): поток попадает автоматически, а «цель может пройти тест
//  A+0, чтобы избежать попадания» — Реакция при этом не тратится. Уклонения и
//  Парирования против потока книга не даёт вовсе. Пока попадание Spray могло
//  быть промахом, секции защиты просто не было; с авто-попаданием hit стал
//  всегда true, и цель начала видеть обе кнопки сразу.
// ════════════════════════════════════════════════════════════════════════════

describe("защита от Распыления", () => {
  it("у Spray только тест на отмену — Уклонения и Парирования нет", () => {
    const html = card({ isMelee: false, wp: { spray: true }, autoHit: "spray" });
    expect(html).toContain("wh-spray-cancel-btn");
    expect(html, "лишняя кнопка сожжёт Реакцию впустую").not.toContain("wh-dodge-btn");
    expect(html).not.toContain("wh-parry-btn");
  });

  it("у обычного стрелкового оружия защита на месте", () => {
