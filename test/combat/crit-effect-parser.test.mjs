...
// таблицей (200 строк) — 31 утверждает смерть напрямую, 0 конфликтов с
...
describe("метрика покрытия — снимок доли распознанного", () => {
  ...
  // wdbc-1rno: 31/200 строк утверждают смерть напрямую (сверено построчно
  // 09.09.2026) — порог с запасом вниз, снимок, не цель роста к 100%
  // (условные «или умереть от шока» — 12 строк — намеренно не в счёте).
  it("крит-таблицы: не менее 25 из 200 строк утверждают смерть напрямую", () => {
    ...
    expect(total).toBe(200);
    expect(deadly).toBeGreaterThanOrEqual(25);
  });
