...
describe("метрика покрытия — снимок доли распознанного", () => {
  ...
  // «убивая» сначала не ловилось, +4 строки после починки) — порог с
  // запасом вниз, снимок, не цель роста к 100% (условные «или умереть от
  // шока» — 12 строк — намеренно не в счёте).
  it("крит-таблицы: не менее 30 из 200 строк утверждают смерть напрямую", () => {
    ...
    expect(total).toBe(200);
    expect(deadly).toBeGreaterThanOrEqual(30);
  });
