...
describe("атака оружием с Распылением", () => {
  it("провальный бросок всё равно даёт попадание и урон", async () => {
    ...
    // Атаки по площади (стр. 34, wdbc-x1nz.2.48): Распыление всегда попадает
    // в Торс — реверс d100 (77 → 77 → «П. Нога») здесь больше не участвует,
    // бросок по-прежнему катается только ради ChatMessage.
...
describe("клин Распыления (стр. 168)", () => {
  it("девятка на первом кубике урона клинит оружие, но попадание в силе", async () => {
    ...
    // Атаки по площади (wdbc-x1nz.2.48): Торс, а не реверс 50 → «05».
