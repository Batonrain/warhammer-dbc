...
describe("recoilItemMultiplier (Malearius/Малеарий)", () => {
  it("без Таланта — ×1", () => {
    const a = actor({ items: [meleeWeapon("Meteor Hammer")], absorption: lightArmor });
    expect(recoilItemMultiplier(a)).toBe(1);
  });

  it("с Талантом, метеоритным молотом и лёгкой бронёй везде — ×2", () => {
    const a = actor({
      items: [talent("Malearius / Малеарий"), meleeWeapon("Meteor Hammer")],
  ...
  it("recoilLimit множит SPD+бонусы предметов, не ОД-прибавку", () => {
    const a = actor({
      items: [talent("Malearius / Малеарий"), talent("Salto / Сальто"), meleeWeapon("Meteor Hammer")],
