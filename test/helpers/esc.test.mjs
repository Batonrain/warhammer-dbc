...
// ── Инвариант 2: имена в любой собираемой разметке экранированы ──────────────
// Диалог видит один ГМ, а чат-карточка уходит всей партии: имя актора или
// предмета задаёт игрок у себя на листе, и разметка в названии исполняется у
// каждого, кому карточка пришла (wdbc-5q7).
describe("имена документов в разметке", () => {
  // Признак разметки — открывающий тег или content: в той же строке. Проверка
  // построчная: шаблонные строки тут многострочные и с вложенными ${`…`},
  // разбирать их ради теста дороже, чем он стоит. Цена — подстановка в строке
  // без тега не проверяется; такие места ловит ревью.
  const HTML_LINE = /<(?:p|b|i|em|strong|div|span|li|ul|ol|option|h[1-6]|img|td|tr|table|label|input|a|form|select|textarea|g)\b|content:|title="/;

  it("везде, где собирается HTML, ${…name} идёт через esc", () => {
    const guilty = [];
    for (const f of FILES) {
      fs.readFileSync(f, "utf8").split("\n").forEach((line, i) => {
        if (!HTML_LINE.test(line)) return;
        for (const [, expr] of line.matchAll(/\$\{([^}]*)\}/g)) {
          if (!/\.name\b/.test(expr) || /\besc\(/.test(expr)) continue;
          guilty.push(`${rel(f)}:${i + 1}`);
        }
      });
    }
...
// ── Инвариант 3: имена в диалогах экранированы ───────────────────────────────
