...
/**
 * Английское/русское имя к сравнимому виду: апостроф, тире, кавычки, регистр,
 * пробелы. Плюс ё→е: книжный текст (PDF-набор) систематически печатает «е»
 * там, где орфографически нужна «ё» («Черного Зеркала», «Темного Письма») —
 * это не расхождение по сути, а типографский вариант той же буквы, найденный
 * при повторном замере 10.09.2026 (резалось ~20% ложных находок инструмента).
 *
 * ВАЖНО про класс апострофов ниже: буквы внутри `[]` должны быть заданы
 * кодами ‘’ (реальные типографские «‘’»), а НЕ визуально похожими
 * символами, вставленными редактором как обычная прямая кавычка — так уже
 * было в первой версии этой функции, и класс из-за этого не ловил ничего
 * (два "разных" символа на экране на деле оказались одним и тем же ',
 * найдено 10.09.2026 при разборе «З'Сатропа» — книга даёт апостроф ’,
 * а замена молча не срабатывала).
 */
...
export function compareOriginalNames(onlyPack = "") {
  ...
  for (const pack of packs) {
    ...
    for (const file of walk(path.join(packsDir, pack))) {
      ...
      // не найденной ни на одном языке. Префикс бывает из НЕСКОЛЬКИХ слов
      // («Система наведения: Стандартная» — в книге просто «Стандартная»),
      // поэтому снимаем всю кириллическую фразу до двоеточия, а не одно слово.
      const russian = normEnText(parts[1].trim()
        .replace(/^[А-ЯЁа-яё][А-ЯЁа-яё\s]*?:\s*/, "")
        .replace(/\(.*?\)/g, " "));
