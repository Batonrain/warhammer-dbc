...
export function hasOriginal(doc) {
  ...
  // wdbc-o30i (16.09.2026): проверка только левой половины ("до /") считала
  // весь weapon-properties/vehicle-equipment "без оригинала" — обе записи
  // были полностью двуязычны, просто в обратном порядке "Русский / English"
  // (см. wdbc-13ce). Проверяем обе половины: оригинал есть, если латиница
  // нашлась хоть с одной стороны разделителя, порядок половин — отдельный
  // вопрос (формат), не вопрос "есть ли оригинал вообще".
  return name.split("/").some(part => LATIN.test(part));
}
