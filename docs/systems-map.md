...
**Длина Оружия (wdbc-x1nz.2.67, 22.09.2026):** книжный раздел про
числовой Rng рукопашного оружия (стр. 39) раньше нигде не читался —
`constants/combat.mjs::meleeEffectiveRange` (база профиля + Хват + Приём:
Выпад +1, Пила →0) и `rules/weapon-length.mjs` (`longerWeaponBonus`,
`chargeTargetDodgeBonus`, `closeQuartersPenalty`). Реализованы 3 из 5
книжных правил: бонус атакующему за более длинное оружие (автогалочка в
`sheets/attack/mods.mjs`), бонус Избегания цели при Натиске на противника
с оружием короче на 3+ (`sheets/attack/selection.mjs::resolveSelection`,
живой пересчёт на сабмите формы), штраф вблизи для Rng≥6. Честно не
смоделировано (отдельные тикеты `wdbc-x1nz.2.67.1`/`.2`): расширенный
Базовый контакт для Rng 8/9 (нужна геометрия карты — намеренно НЕ трогать
общий `rules/tactical-map.mjs::contactType()`, слишком широкий blast radius
ради редкого случая), переменная длина оружия за атаку (нужна новая модель
диапазона в данных + контентная сверка `packs-src`, какие предметы реально
её имеют).
