...
- **Руны Сигиллитов — РЕАЛИЗОВАНО** (wdbc-fsl9, DoomBC «Психокеры-Жабы», стр.
  101-102; исправлена стойкая ошибка этого файла — до 14.09.2026 запись
  ошибочно значилась «ЗАПЛАНИРОВАНО, кода нет», хотя код внесён 10.09.2026).
  `rules/sigillite-runes.mjs` (числовой пул `system.sigilliteRunes {value,max}`:
  старт боя бPR, +бPR/+Forbidden Lore(Archeotech) в начале Хода, потолок
  20+Талант+Археотех; трата бPR психосилы×2, Рунный Удар +4 Руны/+1эPR) +
  `rules/sigillite-runes-combat.mjs`, альтернативный Путь Силы «Руны
  Сигиллитов» поверх обычного конвейера манифестации (`sheets/tabs/
  psychic.mjs`), включаемый Чертой «Магия Сигиллитов»
  (`packs-src/traits/Элитные_архетипы/Сигиллит/`). Пул — не Состояние: у
  записи реестра Состояний (`constants/conditions.mjs`) нет поля под растущий
  максимум, сравнение вариантов см. в шапке `rules/sigillite-runes.mjs`.
