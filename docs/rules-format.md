...
Ошибка здесь не видна ни в коде, ни в тестах, которые считают числа с
подставным актором без правил: галочка приезжает из формы отдельным числом, и
`collectTestMods` складывает её **второй раз**. Так и вышло в психотесте
(порог вырос на 10) и тихо сидело в тесте Страха. Оба случая закрыты тестами в
[situational.test.mjs](../test/rules/situational.test.mjs).
