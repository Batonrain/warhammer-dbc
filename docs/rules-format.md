...
Про `targetHasTrait`: `ctx.target` здесь — актор цели. В матчере ситуативных
модификаторов ([match-context.mjs](../module/rules/match-context.mjs)) то же имя
занято флагом «бросок нацелен». Пока это два разных контекста и они не
пересекаются, но на этапе 2 плана они сходятся в одном объекте, и одно из двух
значений придётся переименовать.
