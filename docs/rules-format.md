...
`skill:psyniscience`, `char:wp`, `power:smite`, `weapon:melee`. Область `all` или
пустая означает «в любом тесте». У `grantValue` и `grantFlag` в том же поле лежит
имя: производного поля («reactions», «sizeMod») или возможности.
...
одна запись означала бы два разных правила книги. Области `power:` и `weapon:` не
совпадают ни с чем, пока атаки не переведены на конвейер (шаг 5.2 плана).
