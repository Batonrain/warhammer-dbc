# Формат правил

Справочник к этапам 1–3 [плана](architecture-plan.md). Описывает, как записать
игровое правило данными вместо кода.

## Зачем это нужно

Сейчас правило существует как русский текст:

```js
// module/constants/races.mjs
{ name: "Nimble / Проворный",
  benefit: "Атакующим по нему −A.b к попаданию.",
  effects: {} }
```

Человек прочитает, система применить не сможет. Поэтому каждое действующее правило
пишется отдельно, кодом, и расходится с текстом при первой же правке.

Формат ниже добавляет машинную часть **рядом** с текстом. Текст остаётся описанием
для игрока и не удаляется никогда.

## Форма записи

```js
{
  id:        "eldarten",              // обязателен, уникален в системе
  label:     "Eldarten / Эльдарское Тело",
  when:      { /* условия */ },       // пусто = действует всегда
  effects:   [ /* эффекты */ ],
  overrides: [ "id_другого_правила" ] // необязательно
}
```

`id` пишется в нижнем регистре, точка разделяет уровни: `smite.astartes`,
`aeldari.path.warlock`. По этому идентификатору правило вытесняется другим, поэтому
менять его после выхода релиза нельзя, как нельзя менять имя поля в базе.

## Условия (`when`)

Правило действует, когда истинны **все** условия одновременно. Пустой `when`
означает «всегда».

### `anyOf` — «ИЛИ» между ветками

`anyOf` — единственный ключ `when`, который не предикат, а СВЯЗКА: список
веток, из которых достаточно одной. Внутри ветки условия складываются
через «И», и сам `anyOf` соседствует с обычными условиями тоже через «И»:

```js
// «Астартес И (Ультрамарин ИЛИ Железная Рука)» — одним правилом, как в книге
when: {
  race: ["astartes"],
  anyOf: [{ subrace: ["ultramarines"] }, { subrace: ["ironhands"] }]
}
```

Пустой `anyOf: []` не пропускает никого. Это не «условия нет»: автор написал
ИЛИ и не заполнил ни одной ветки, и тихо пропустить всех было бы
противоположностью написанному.

У записей Конструктора есть своё «ИЛИ» — галочка «достаточно любого
условия» (`entry.when.anyOf`, `module/rules/mech-when.mjs`). Там оно связывает
восемь гейтов записи, а не ветки, и считаются только ЗАПОЛНЕННЫЕ гейты:
незаполненный считается пройденным, и наивное ИЛИ по всем восьми пропускало
бы вообще всё. Два языка условий пока остаются разными (wdbc-n48f) —
сведено только «ИЛИ», с которого тот тикет и велел начинать.

| Ключ | Значение | Истинно, когда | Есть |
| --- | --- | --- | --- |
| `race` | массив ключей рас | раса актора в списке | да |
| `subrace` | массив ключей субрас | субраса актора в списке | да |
| `alignment` | массив мировоззрений | мировоззрение актора в списке | нет |
| `sizeMax` | число | итоговый Размер не больше указанного | да |
| `woundTier` | массив (`healthy`/`light`/`heavy`/`dying`) | Уровень Ранения актора в списке | да |
| `sizeMin` | число | итоговый Размер не меньше указанного | нет |
| `charMin` | объект `{ ключ: число }` | все указанные характеристики не ниже порога | да |
| `charBonusMin` | объект `{ ключ: число }` | Бонусы указанных характеристик не ниже порога (`{ s: 5 }` — «S.b 5+»); не то же, что `charMin` — Бонус несёт ступени Unnatural | да |
| `hasTalent` | строка или массив | у актора есть талант с таким именем | да |
| `hasTrait` | строка или массив | у актора есть черта с таким именем | да |
| `psyRatingMin` | число | пси-рейтинг (`system.psyker.rating`) не ниже указанного | да |
| `pathGradeMin` | объект `{ group?, key?, grade }` | достигнута градация Пути Азуриан не ниже указанной: `{ group: "Путь Воина", grade: "next" }` — любой из десяти Путей Воина, `{ key: "banshee", grade: "master" }` — конкретный. Группа есть только у Путей Воина и Путей Видящих: остальные пятнадцать стоят в книге самостоятельными разделами | да |
| `geneSeedLegion` | массив id легионов (римская цифра, `"VIII"`) | легион Геносемени актора в списке | да |
| `weaponClass` | массив | класс оружия в контексте броска подходит | да |
| `discipline` | массив | дисциплина психосилы в контексте подходит | нет |
| `targetHasTrait` | строка или массив | у цели есть такая черта | да |
| `hasFaction` | строка или массив | актор состоит в такой фракции или в нижестоящей | да |
| `targetHasFaction` | строка или массив | то же про цель броска | да |
| `charNotIn` | массив ключей характеристик | характеристика ТЕКУЩЕГО теста (`ctx.char`) не входит в список | да |
| `charIn` | массив ключей характеристик | характеристика ТЕКУЩЕГО теста (`ctx.char`) входит в список (обратное `charNotIn`) | да |
| `hasCondition` | массив ключей `CONDITIONS_DEF` | у актора есть хотя бы одно из перечисленных Состояний | да |
| `targetHasCondition` | массив ключей `CONDITIONS_DEF` | то же про цель броска | да |
| `targetLacksCondition` | массив ключей `CONDITIONS_DEF` | у цели нет НИ ОДНОГО из перечисленных Состояний | да |
| `isBlinded` | `true`/`false` | актор Ослеплён: свой флаг или потеряны оба глаза (Sonar Sense/Unnatural Senses снимаются вытеснением, не здесь) | да |
| `rankAndFile` | `true`/`false` | актор — «Рядовой» (`system.rankAndFile`, корбук стр. 4); у чемпионских Черт расы стоит `false` | да |
| `inPariahVoid` | `true`/`false` | актор в ауре Парии — у него Черта-метка «In the Pariah's Void» (rules/null-zones.mjs); по Черте, не по флагу, чтобы не зациклить сбор правил | нет |
| `isDaemon` | `true`/`false` | актор — демон: тип `daemon`/`demonPrince` или Черта Daemonic | нет |
| `targetPsykerOrDaemon` | `true`/`false` | цель броска — псайкер (Пси-Рейтинг ≥ 1) или демон | да |

Столбец «Есть» — реализовано ли условие сейчас. Восемь предикатов покрывают
существующий контент, остальные добавляются, когда под них появляются данные.
Правило с нереализованным условием не действует и жалуется в консоль, как при
опечатке: молча не сработать оно не может.

Одиночное значение везде читается как список из одного элемента: `race: "human"`
и `race: ["human"]` — одно и то же.

Подпись предиката всегда одна:

```js
// module/rules/predicates.mjs
export const PREDICATES = {
  race:    (actor, ctx, value) => list(value).includes(actor?.system?.race),
  sizeMax: (actor, ctx, value) => sizeOf(actor) <= Number(value),
  charMin: (actor, ctx, value) => Object.entries(value ?? {}).every(
             ([k, n]) => (Number(actor?.system?.characteristics?.[k]?.total) || 0) >= n),
  weaponClass: (actor, ctx, value) => list(value).includes(ctx?.weapon?.system?.weaponClass)
};
```

Про Размер: у актора их два поля. Базовый лежит в `system.size`, вклад Черт — в
`system.sizeMod`, сумму лист кладёт в `system.sizeTotal`. `sizeOf` берёт готовую
сумму, а без неё складывает сам, потому что у подставного актора в тесте
посчитанного поля нет.

Про `hasTalent` и `hasTrait`: список означает «и». Правило, которому подходит
любой из двух Талантов, пишется двумя правилами; правило, требующее оба сразу,
иначе не записать. Оба предиката ищут и среди Талантов, и среди Черт, а имя
сравнивают по любой половине двуязычного названия: `"Gene-Seed"` и
`"Геносемя"` найдут один и тот же предмет, а `"Resistance"` найдёт
`"Resistance (Cold) / Сопротивление (Холод)"`.

Про `hasFaction` и `targetHasFaction`: фракция — предмет на листе, поэтому актор
может состоять сразу в нескольких (астартес-предатель — и Хаос, и свой легион).
Отбор односторонний: условие «Хаос» подходит роте в составе его легиона, условие
«III рота» весь Хаос не покрывает. Дерево строится по ссылке `parent` у самих
Фракций и живёт в реестре ([factions.mjs](../module/rules/factions.mjs)),
который наполняет сторона Foundry при готовности мира — до этого условие просто
не срабатывает.

У `targetHasFaction` два источника, и они не пересекаются: в бою цель это
выделенный токен (`ctx.targetActor`), а в социальном тесте токена нет вовсе —
«Инквизицию» мышью не выделишь, — поэтому игрок выбирает фракцию собеседника в
диалоге, и она приезжает в `ctx.socialFaction`. Выбор в диалоге важнее токена.

Про `targetHasTrait`: актор цели лежит в `ctx.targetActor`. Имя `ctx.target` в том
же объекте занято флагом «бросок нацелен» — его читает матчер ситуативных
модификаторов ([match-context.mjs](../module/rules/match-context.mjs)), и на этапе
2 оба контекста сошлись в одном объекте. Поэтому переименован предикат, а не
данные: галочки с `when.target` лежали в Особенностях Происхождений, и
переименование в них потребовало бы миграции. (Та галочка, ради которой это
делалось, — «Паранойя Выжившего» Мира смерти — с тех пор стала возможностью
`attack.surpriseImmune`, см. «Возможности» ниже; сам флаг матчера остался.)

Три требования к предикату, и они не обсуждаются:

1. **Чистая функция.** Никаких обращений к `game`, `ui`, `canvas`, никаких бросков.
   Иначе правило не протестировать без запуска Foundry, а весь смысл был в этом.
2. **Безопасен к отсутствию данных.** У подставного актора в тесте может не быть
   половины полей. Отсюда `?.` и `?? 0` в каждом обращении.
3. **Возвращает строго `true` или `false`.** Не `undefined`, не число.

Неизвестный ключ условия даёт ошибку в консоль и `false`. Опечатка в данных не
должна проходить молча: тихо не сработавшее правило ищется днями.

## Эффекты

| `kind` | Поля | Что делает |
| --- | --- | --- |
| `charBonus` | `target`, `value` | плюс к бонусу характеристики (Unnatural) |
| `charTotal` | `target`, `value` | плюс к значению характеристики |
| `rollBonus` | `target`, `value` | модификатор к тесту; `auto: true` — без галочки, см. ниже |
| `rollMode` | `target`, `mode`, `rolls` | несколько бросков с выбором (`keepBest`, `keepWorst`) |
| `penaltyMul` | `target`, `factor` | множитель штрафов, `0.5` = половина |
| `apBonus` | `target`, `value` | броня по локации или `all` |
| `damageBonus` | `target`, `value` | плюс к урону |
| `damageDice` | `target`, `value` | замена формулы урона |
| `grantValue` | `target`, `value` | плюс к производному полю (реакции, скорость) |
| `grantFlag` | `target` | возможность по имени, см. «Возможности» ниже |
| `grantAptitudeOverride` | `scope`, `match`, `align` | Навык/Талант/Характеристика всегда Дружественный/Враждебный независимо от Покровительства, см. ниже |
| `fearRating` | `value` | рейтинг страха, берётся максимум, не сумма |
| `grantItem` | `uuid`, `qty` | выдать предмет при получении источника |
| `critRangeMod` | `target`, `side`, `value` | шире диапазон Критического Успеха/Провала (стр. 25) на `value` натуральных чисел; `side`: `success`, `failure` или `both` |
| `grantWeaponProp` | `target`, `propKey`, `rating`, `rating2` | доп. Особое Свойство Оружия на эту атаку, см. ниже |
| `failDegMod` | `target`, `value` | доп. степени провала (`value`, может быть отрицательным), суммируются, только если тест УЖЕ провален |
| `scriptTrigger` | `target`, `side`, `itemId`, `entryId` | автозапуск записи `kind:"script"` по исходу теста; `side`: `critSuccess` или `critFailure` |
| `autoFail` | `target` | тест в этой области провален без учёта броска (Ослеплён — `basedon:bs`); применяет общий исход теста `rules/kind-outcome.mjs` |
| `script` | `code` | аварийный выход, см. предупреждение ниже |

`target` для бросков пишется с областью через двоеточие: `initiative`,
`skill:psyniscience`, `char:wp`, `power:smite`, `attack`, `weapon:melee`. Область
`all` или пустая означает «в любом тесте». Есть и `social` — все социальные
навыки книги разом (Обаяние, Командование, Коммерция, Обман, Дознание, Допрос,
Запугивание). Своего перечня у неё нет: признак берётся из таблицы навыков
(`apt2: "social"`), иначе два списка разошлись бы при добавлении навыка.
Проницательность помечена общей и сюда не входит — так в книге. У `grantValue` и `grantFlag` в том же
поле лежит имя: производного поля («reactions», «sizeMod») или возможности.

`basedon:<хар-ка>` — любой тест навыка или характеристики на этой
Характеристике (`basedon:ws` — и тест WS, и Парирование); атаку и манифестацию
не подхватывает, у них свои области. `target` может быть и списком областей —
эффект срабатывает один раз, если подошла хоть одна (Ослеплён: `["basedon:ws",
"skill:awareness"]` не даёт −60 Бдительности на WS).

Области атаки, появившиеся на шаге 5.2:

| Область | Куда попадает |
| --- | --- |
| `attack` | любой удар и выстрел |
| `weapon:melee` | рукопашные, включая метательные |
| `weapon:ranged` | стрелковые |
| `weapon:<класс>` | конкретный класс: `pistol`, `basic`, `heavy`, `launcher`… |

Области манифестации, появившиеся на шаге 5.4:

| Область | Куда попадает |
| --- | --- |
| `power` | любая манифестация психосилы |
| `power:<имя>` | конкретная сила |

Имя силы сравнивается так же, как имена в `hasTalent`: по любой половине
двуязычного имени, со снятой специализацией в скобках. `power:smite`,
`power:порицание` и `power:Smite` — одно и то же.

Область сопоставляется с контекстом броска в
[resolve-test.mjs](../module/rules/resolve-test.mjs). Тест навыка и тест
характеристики различаются наличием `ctx.skill`, поэтому `char:int` действует
только на тест Интеллекта и не подхватывается броском навыка на Интеллекте: иначе
одна запись означала бы два разных правила книги. По той же причине атака
отбирается отдельной веткой и не подхватывает `char:ws`: «+10 к тестам
Оружейного Мастерства» и «+10 ко всем ударам» — два разных правила книги.
Манифестация отбирается третьей веткой по той же причине: психотест идёт по Воле
(а Прорицание — по Псинауке), но «+10 к тестам Воли» и «+10 к манифестациям» —
снова два разных правила.

### Значение, зависящее от цели

Иногда числа в данных быть не может: «Проворный» даёт атакующему минус Бонус
Ловкости **цели**, а он у каждой цели свой. Такие правила вместо `value` пишут
`valueFrom`, и значение считается на каждый бросок:

```js
effects: [{
  kind: "rollBonus",
  target: "attack",
  valueFrom: { targetCharBonus: "ag", multiplier: -1 }
}]
```

Кроме `targetCharBonus` есть `selfConditionLevel: "<ключ Состояния>"` —
уровень своего Состояния со счётчиком («−5 × уровень Обескровливания»,
`multiplier: -5`). Источник `targetCharBonus` — бонус характеристики цели;
`multiplier` необязателен и по умолчанию равен 1. Неизвестный источник не
превращается молча в ноль, а жалуется в консоль, и галочка не появляется.

Цель берётся из `ctx.targetActor` — того же поля, что читает предикат
`targetHasTrait`. Если цель не выбрана, значение равно нулю, но правило с
условием по цели до этого и не доберётся: отбор его не пропустит.

До диалога броска доходят два вида эффектов: `rollBonus` даёт галочку со своим
значением, `penaltyMul` с `factor: 0.5` — галочку «ополовинить штраф». Другого
множителя диалог не умеет, и правило с ним не применяется молча, а жалуется в
консоль. Остальные виды подключаются вместе с фазами 5–6 конвейера.

### Модификатор без галочки (`auto`)

`rollBonus` с `auto: true` не спрашивает игрока, а применяется сам и показан
строкой в окне броска («Состояние (учтено в Пороге)»). Так записаны ситуативные
штрафы состояния тела и снаряжения — Усталость, Марш, снятый шлем силовой брони,
выключенная силовая броня, Перевес инвентаря
([situational.mjs](../module/rules/situational.mjs)):

```js
effects: [{ kind: "rollBonus", target: "all", value: -10, label: "😓 Усталость", auto: true }]
```

Отличие от обычного `rollBonus` не в силе эффекта, а в том, есть ли тут выбор.
«Уместен ли здесь бонус Черты» решает игрок; «устал ли персонаж» не решает
никто — это его состояние, и оно действует, пока действует. Тот же принцип, что
у `critRangeMod` и `grantWeaponProp` выше.

`resolveTest` отдаёт эти модификаторы отдельным списком `autoMods`, а не
подмешивает к `mods`, по двум причинам сразу: галочки складываются в поле
«Модификатор» и попадают под «ополовинить штраф», а автоматические прибавляются
к Порогу уже после ополовинивания (смешай их — и число за столом поменяется); и
всякий существующий читатель `mods` продолжает видеть ровно то, что видел, не
задваивая ничего.

**Какой сборщик брать на месте броска.** Их два, и путать нельзя
([roll-mods.mjs](../module/rules/roll-mods.mjs)):

| Сборщик | Когда | Что складывает |
| --- | --- | --- |
| `collectTestMods` | у места **нет** диалога с галочками (кнопка в карточке, Реакция) | автоштрафы **и** галочки — спрашивать игрока негде |
| `autoTestMods` | диалог с галочками **уже есть** (тест Страха, психотест, атака) | только автоштрафы |

Ошибиться можно в обе стороны, и вторая ошибка хуже первой:

- взять `collectTestMods` там, где диалог есть — галочка приезжает из формы
  отдельным числом и складывается **второй раз**. Так вышло в психотесте
  (Порог вырос на 10) и тихо сидело в тесте Страха;
- взять `autoTestMods` там, где диалога **нет** — всё, что реестр даёт тесту,
  **молча теряется**. Так вышло у удара оружием в упор
  ([attack-dialog.mjs](../module/sheets/attack-dialog.mjs)::`showAttackDialogNoWeapon`):
  бросок там безусловный, по клику, и «+10 к любому удару» с Черты не доезжал
  никуда.

Задвоение за столом замечают сразу, потерю — никогда: Порог просто чуть
меньше, чем должен быть, и списать это не на что. Поэтому оба случая закрыты
тестами в [situational.test.mjs](../test/rules/situational.test.mjs).

**Признак, по которому выбирать:** есть ли у ЭТОГО пути броска форма, где
игрок жмёт кнопку. Не «есть ли диалог в файле» — в
[attack-dialog.mjs](../module/sheets/attack-dialog.mjs) их два, и у соседних
функций ответ разный.

### Чего не должен делать источник правил

**Источник не спрашивает у самого реестра и не тянет за собой лист.** Оба
запрета — про один и тот же круг: сбор правил, зовущий сбор правил.

Живой пример — Усталость. Её расчёт спрашивал возможности актора
(`hasRuleFlag`), а те собирают правила заново: источник оказался вызван изнутри
самого себя. Это ломается дважды. В расчёте — бесконечной рекурсией; от неё
сборка ([collect.mjs](../module/rules/collect.mjs)) защищена сама: повторный
вход того же источника по ТОМУ ЖЕ актору пропускается, ответ был бы тот же,
только глубже (вложенная сборка по ДРУГОМУ актору — Адъютант смотрит Командира
— законна и работает). В импортах — кругом
`sources → источник → … → flags → collect → sources`, на котором ES-загрузчик
встаёт насмерть; от этого защиты нет и быть не может, круг нужно не замыкать.

Отсюда два правила для нового источника:

1. **Читать состояние актора напрямую**, а не через `hasRuleFlag`. Записи
   Конструктора сканируются по предметам — так сделаны терпимость к Усталости
   ([fatigue-grace.mjs](../module/rules/fatigue-grace.mjs)), Зависимость
   ([addiction.mjs](../module/rules/addiction.mjs)) и Мутации, меняющие
   Усталость ([situational.mjs](../module/rules/situational.mjs)).
2. **Импортировать только из `rules/`, `constants/` и `helpers/`.** Понадобился
   расчёт, живущий в `combat/` или `sheets/` — переносить его чистую часть в
   `rules/`, оставляя на прежнем месте реэкспорт. Так переехали Усталость с
   Маршем (из `sheets/tabs/conditions.mjs`) и штраф выключенной брони (из
   `combat/armor-mods.mjs` в
   [armour-penalty.mjs](../module/rules/armour-penalty.mjs)).

Про `fearRating`: складывать нельзя. Два источника с рейтингом 2 дают 2, а не 4.
Это уже учтено в существующем коде
([effect-keys.mjs](../module/constants/effect-keys.mjs), тип `upgrade`), и новый
реестр должен вести себя так же.

Про `script`: свободный JS в правиле возвращает нас туда, откуда мы уходили.
Допустим только когда правило действительно нельзя выразить данными, и в этом
случае к нему пишется комментарий с объяснением, почему.

Про `critRangeMod`: диапазон по умолчанию 1–5/96–100, каждое очко `value`
расширяет его на одно натуральное число с указанной стороны. Талант,
расширяющий Критический Успех на тестах Медицины до 1–10:

```js
effects: [{ kind: "critRangeMod", target: "skill:medicae", side: "success", value: 5 }]
```

Считается в `rules/resolve-test.mjs::critModsFromRules`, применяется в
`rules/roll-outcome.mjs::criticalOutcome`. В отличие от `rollBonus` это не
галочка для игрока — диапазон не выбирают, он просто шире, пока правило
действует.

## Особое Свойство Оружия от правила (`grantWeaponProp`)

Движок сборки Особых Свойств Оружия (`combat/weapon-properties.mjs::resolveWeaponPropsList`/
`aggregateAuto`, собирающий их в `combat/attack.mjs`/`sheets/attack-dialog.mjs`) до
wdbc-w8z4 читал только состояние атакующего/оружия/боеприпаса
(`system.weaponProps` + хват/боеприпас/условные свойства боеприпаса,
`combat/attack-weapon.mjs::mergeExtraProps`). Не было пути для правила вроде
«цель помечена → атаки союзников по ней получают Proven(3)» — Особое Свойство
нельзя было выдать из-за состояния **цели**, только числовой модификатор
(`rollBonus`).

`grantWeaponProp` закрывает этот пробел тем же путём, что `rollBonus`:

```js
effects: [{ kind: "grantWeaponProp", target: "attack", propKey: "toxic", rating: 1 }]
```

`propKey` — ключ из `constants/weapon-properties.mjs::WEAPON_PROPERTIES` (тот же
словарь, что и `system.weaponProps[].key` у оружия). Неизвестный ключ — ошибка в
консоль, запись не идёт в атаку, как и опечатка в `kind`/`when`. `rating`/`rating2`
необязательны (по умолчанию 0) — Свойства без рейтинга (Piercing, Flame и т.п.)
их не используют.

В отличие от `rollBonus` эффект **не превращается в галочку диалога**: он
доливается в список Особых Свойств безусловно, как только правило прошло отбор
по `when` (тот же принцип, что у `critRangeMod` — «не выбирают, оно просто есть,
пока правило действует», а не число, которое может случайно задвоиться с чем-то
ещё). Собирается `rules/resolve-test.mjs::weaponPropsFromRules`, доезжает до
`resolveTest(...).weaponProps`; `sheets/attack-dialog.mjs` доливает результат в
свой список Особых Свойств через `combat/attack-weapon.mjs::mergeExtraProps`
(параметр `ruleProps`, тот же мёрж «больший рейтинг побеждает», что у боеприпаса)
ДО расчёта `aggregateAuto` — иначе порог/урон/памятки в диалоге не увидели бы
свойство вовсе. Тем же параметром `ruleProps` итог доезжает через `opts` диалога
до `combat/attack.mjs::_executeAttackRoll`, который и правда исполняет бросок:
диалог для атаки нужен, чтобы прочитать `ctx.targetActor` (сама функция броска
цели не знает, см. комментарий у `ruleProps` в `attack.mjs`).

Область (`target`) — та же, что у `rollBonus`: `attack`, `weapon:melee`,
`weapon:ranged`, `weapon:<класс>`. Условие обычно читает `ctx.targetActor`
(`targetHasTrait` и подобные, cross-actor метка вроде Hex-Marked Prey/Проклятая
Метка), но `when` может быть и про самого актора — общего требования нет.

Про `failDegMod` (wdbc-1rno, «Разумная Циста»: «провалив тест на социальные
взаимодействия, персонаж получает ещё +3 Провала»): считается уже ПОСЛЕ броска,
а не до него — модификатор к тесту (`rollBonus`) меняет число до сравнения с
Порогом, `failDegMod` меняет степень провала уже посчитанного исхода, и только
если сам тест провален (успешный тест `failDegMod` не трогает вовсе). Тоже не
галочка, как и `critRangeMod` — суммируется безусловно, пока правило действует
(книга не даёт игроку выбора «хочу ли я похуже провалиться»). Считается в
`rules/resolve-test.mjs::failDegModFromRules`, применяется в
`rules/kind-outcome.mjs::resolveKindOutcome`.

```js
effects: [{ kind: "failDegMod", target: "social", value: 3 }]
```

Про `scriptTrigger` (wdbc-1rno, «Полимат»: «когда он выбрасывает Критический
Успех на таком тесте, он получает 1d5 Усталости... и сразу же делает ещё один
тест») — некоторые последствия Крита нельзя выразить одним числом (получить
Усталость, начать новый тест, что угодно ещё) и годится только произвольный
JS, а не декларативная запись. `scriptTrigger` НЕ несёт сам код: код живёт на
записи `kind:"script"` того же предмета (поле `code`, `scriptTrigger:
"critSuccess"|"critFailure"`, область — `modScope` тот же, что у `testMod`),
а эффект несёт только АДРЕС этой записи (`itemId`/`entryId`) — чистые данные,
без ссылки на живой документ, тот же принцип, что у `grantItem`/`uuid`.
Отбор подходящих по области — `rules/resolve-test.mjs::scriptTriggersFromRules`
(та же `effectAppliesTo`, что у прочих `target`); сверка стороны (реальный
`crit.success`/`crit.failure` известен только после броска) и сам запуск —
`rules/kind-outcome.mjs::resolveKindOutcome` (переиспользует
`apps/mechanics.mjs::scriptRunReady`/`markScriptRunUsed` — тот же throttle,
что у ручной кнопки «▶ Запустить», и `apps/item-script.mjs::executeItemCode`
— тот же исполнитель):

```js
// на предмете, entry.id совпадает с scriptTrigger.entryId:
{ kind: "script", scriptTrigger: "critSuccess", modScope: "skill", skillKey: "trade",
  code: `await actor.update({"system.fatigue.value": actor.system.fatigue.value + (await new Roll("1d5").evaluate()).total});` }

// эффект правила, построенный из этой записи (item-rules.mjs):
effects: [{ kind: "scriptTrigger", target: "skill:trade", side: "critSuccess", itemId: "...", entryId: "..." }]
```

Ручной режим (`scriptTrigger: ""`, по умолчанию) правил вообще не даёт — та
же запись `kind:"script"` остаётся доступна только кнопкой «▶ Запустить» на
листе предмета, как раньше; оба режима не исключают друг друга буквально
(поле одно), но семантически это разные способы вызвать один и тот же код.

## Возможности

Часть правил книги не меняет числа, а разрешает: «лечится как космодесантник»,
«доступна папка талантов Геносемени». В коде такое жило проверкой расы, и вторая
раса с тем же свойством требовала правки кода. Эффект `grantFlag` выдаёт
возможность по имени, а код её спрашивает:

```js
// module/rules/flags.mjs
import { hasRuleFlag } from "../rules/flags.mjs";

if (hasRuleFlag(patient, "healing.astartes")) { /* … */ }
```

Имя возможности — название правила книги, а не расы, даже когда раса в нём
упомянута. `healing.astartes` читается как «лечится как космодесантник», и Черта
Арлекина «Искажённое Тело» с той же оговоркой получит ту же строку из своих
данных, не тронув код листа.

Возможности, которые кто-то спрашивает сейчас:

| Имя | Что открывает | Кто спрашивает |
| --- | --- | --- |
| `healing.astartes` | пассивное лечение считается Отдыхом, Отдых — Постельным режимом | `_applyHealing` |
| `unarmed.astartesProfile` | усиленный профиль безоружного удара (стр. 40) | `_showAttackDialogNoWeapon` |
| `talents.geneSeed` | папка талантов «Геносемя» в пикере | `_talentGroupLock` |
| `fate.save` | трата Очка Судьбы/Бесчестья может не состояться (1d10, при 1 возврат) | хук `updateActor`, [fate-save.mjs](../module/rules/fate-save.mjs) |
| `assist.beyondCap` | помощник встаёт сверх лимита и слот не занимает | `assistRejection` |
| `attack.surpriseImmune` | по носителю не работает «Цель Врасплох» (+30) | `showAttackDialog` |
| `fear.faithInThePast` | кнопка «Вера в прошлое» в карточке проваленного Страха | `_executeFearRoll` |

Последние четыре — Особенности Происхождений
([library/homeworlds.mjs](../module/rules/library/homeworlds.mjs)). Они
показывают, зачем возможности вообще нужны: каждая вмешивается в СВОЮ точку
системы, и ни одна не выражается ни модификатором, ни выдачей предмета. Раньше
такие Особенности жили именованными ключами со своим сканом предметов
(`kind:"specialRule"` на ветке «Конструктор-и-происхождения»); отдельный
механизм рядом с реестром правил не понадобился — хватило `grantFlag`.

Возможность, гасящая чужой модификатор, спрашивается **у цели, без контекста
броска**: контекст описывает бросок атакующего, и правило цели про чужое оружие
ничего не знает. Гашеное показываем зачёркнутым, а не убираем из списка — игрок
должен видеть, ПОЧЕМУ бонуса нет. Подпись правила-источника для таких мест даёт
`ruleFlagLabels(actor, flag)`.

Разовые возможности («один раз за столкновение») отмечаются на самом акторе:
`flags.warhammer-dbc.usageLimits.<возможность>` — предмета-носителя у правила
нет, поэтому предметная метка `usageLimit` тут не годится. Обе сбрасывает
кнопка «Новая сцена» ([game-session.mjs](../module/apps/game-session.mjs)).

Два правила про имена. Первое: новое имя заводится вместе с местом, которое его
читает, иначе флаг молча ничего не делает. Второе: возможность подчиняется отбору
как любой эффект — правило, не прошедшее `when`, её не выдаёт, иначе флаг стал бы
просто вторым названием расы.

## Override склонности (`grantAptitudeOverride`)

Раса/субраса иногда объявляет Навык, Талант или Характеристику всегда
Дружественными или Враждебными «независимо от Покровительства» — тем же
языком книги, что и культура легиона Космодесанта (`cultureCat()`,
[legions.mjs](../module/constants/legions.mjs)), но источник другой: не
культура ордена, а сама раса/субраса (Африэль/Эльданар — N характеристик и
навыков по выбору игрока; Серый Человек — целые ветки Скорость/
Внимательность/Избегание, wdbc-zk69). Эффект:

```js
effects: [{ kind: "grantAptitudeOverride", scope: "skill", match: "Deceive", align: "ally" }]
```

`scope` — `"skill"` | `"talent"` | `"characteristic"`. `match` для
характеристики сравнивается ТОЧНО с её ключом (`ws`, `bs`, …) — ключей всего
девять, подстрока только путала бы. Для навыка/таланта — подстрокой по имени
из компендиума, тем же приёмом, что у `cultureCat`: «Hatred (Imperial
Fists)» ловится записью «Hatred», а `"группа:Скорость"` — целой веткой
(специализация/папка совпадает с именем группы целиком). `align` —
`"ally"` | `"enemy"`; при конфликте нескольких записей враждебность
побеждает (тот же приём, что внутри `cultureCat`).

Собирается [aptitude-overrides.mjs](../module/rules/aptitude-overrides.mjs)
(`resolveAptitudeOverride`) через тот же `collectRules`, что и `hasRuleFlag`.
Приоритет **выше** культуры легиона — вызывающий код (`module/sheets/tabs/
advance.mjs`, `module/sheets/item-picker.mjs`, `module/apps/duplicate-
refund.mjs`) проверяет override раньше `cultureCat()` и передаёт результат
как `cultCat` в `charCostXP`/`skillCostXP`/`talentCostXP`
([advancement.mjs](../module/constants/advancement.mjs)) — раса
фундаментальнее полковой культуры, а текст субрас говорит то же «независимо
от Покровительства».

В Конструкторе Механики это второй режим записи «Возможность»
(`kind:"capability"`, `capabilityMode:"aptOverride"` вместо обычного
`capabilityKey`) — режим переключает автор, а не заполненность полей, иначе
переключение назад на обычную Возможность оставляло бы «висячий» override.

## Вытеснение (`overrides`)

Механизм, ради которого затевался весь план. Правило может отменить другое по его
идентификатору. Это и есть ответ на «у эльдар этот талант работает иначе» и «у
Астартес своя версия психосилы».

```js
// Базовая версия
export const SMITE = {
  id: "smite.baseline",
  effects: [{ kind: "damageDice", target: "power:smite", value: "1d10" }]
};

// Версия Астартес вытесняет базовую
export const SMITE_ASTARTES = {
  id: "smite.astartes",
  overrides: ["smite.baseline"],
  when: { race: ["astartes"], hasTrait: "Gene-Seed" },
  effects: [{ kind: "damageDice", target: "power:smite", value: "1d10+PR" }]
};
```

У астартеса действует вторая, у человека первая. Форк кода не нужен, обе версии
лежат рядом и обе читаемы.

Порядок разбора в `collectRules`: сначала собрать все правила от всех источников,
потом отобрать по `when`, и **только потом** снимать вытесненные. Наоборот нельзя:
правило с невыполненным условием не должно ничего вытеснять.

Если два прошедших отбор правила вытесняют друг друга, это ошибка в данных.
Пишем в консоль и оставляем оба: молча выбирать одно из двух хуже, чем показать
двойной эффект, который сразу заметят.

## Источники правил

Правила приходят из разных мест, а собираются в один список.

```js
// module/rules/sources.mjs
registerRuleSource("race",      a => RACES[a.system.race]?.rules ?? []);
registerRuleSource("subrace",   a => SUBRACES[a.system.subrace]?.rules ?? []);
registerRuleSource("homeworld", a => HOMEWORLD_BY_KEY[hwKey(a)]?.rules ?? []);
registerRuleSource("path",      a => pathRules(a));
registerRuleSource("items",     a => a.items.flatMap(i => i.system.rules ?? []));
```

Добавить книгу означает зарегистрировать источник и положить данные. Ядро не
меняется. Реестр подсистем
[features.mjs](../module/constants/features.mjs) уже умеет включать и выключать
книги, и выключенная книга должна убирать свои правила из сборки автоматически.

## Полный пример

Черта эльдар «Eldarten» из
[races.mjs](../module/constants/races.mjs). В книге это абзац на десять строк.
Машинная часть:

```js
export const ELDARTEN = {
  id: "aeldari.eldarten",
  label: "Eldarten / Эльдарское Тело",
  when: {
    race: ["azuriane", "drukhari", "harlequin", "exodite"],
    sizeMax: 1                       // «теряется при Размере 2+»
  },
  effects: [
    { kind: "grantValue", target: "reactions",  value: 1 },
    { kind: "rollBonus",  target: "initiative", value: 4 },
    { kind: "rollMode",   target: "initiative", mode: "keepBest", rolls: 3 },
    { kind: "penaltyMul", target: "skill:psyniscience", factor: 0.5 }
  ]
};
```

Оговорка «теряется при Размере 2+» стала условием `sizeMax: 1`, а не примечанием,
которое надо помнить. Остальные пункты черты (иммунитет к людским болезням,
долгожительство, отношение слаанешитских ветвей) в машинную часть не переводятся:
их применяет мастер, и они остаются в тексте `benefit`. Переводить в данные надо
то, что система считает, а не всё подряд.

## Как проверять

Правило тестируется на литерале, без Foundry:

```js
import { describe, it, expect } from "vitest";
import { matchRule } from "../module/rules/collect.mjs";
import { ELDARTEN } from "../module/rules/library/aeldari.mjs";

const actor = (over = {}) => ({
  system: { race: "azuriane", sizeMod: 0, characteristics: {}, ...over },
  items: []
});

describe("Eldarten", () => {
  it("действует у азуриан обычного размера", () => {
    expect(matchRule(ELDARTEN, actor(), {})).toBe(true);
  });

  it("не действует при Размере 2", () => {
    expect(matchRule(ELDARTEN, actor({ sizeMod: 2 }), {})).toBe(false);
  });

  it("не действует у людей", () => {
    expect(matchRule(ELDARTEN, actor({ race: "human" }), {})).toBe(false);
  });
});
```

Три теста на правило: подходит, не подходит по основному условию, не подходит по
расе. Этого достаточно, чтобы правило не сломалось молча.
